...
`use*.ts` is a data hook, `*.tsx` is a view, `wire/api.ts` is the generated wire model; in C#, `*Service` /
...
- **C#** — a `Contracts/` unit in the assembly, with cross-cutting enums defined once in a neutral
  vocabulary file. Declare an enum's wire spelling on the enum TYPE, never on a serializer options
