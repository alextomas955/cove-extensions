...
export default tseslint.config(
  ...
  // the rule's suggested `import` is not a substitute: that file only declares modules and emits
  // nothing, so importing it would add a runtime import of a module that does not exist. TypeScript
  // honors a reference directive only above the first statement, which leaves no room for an inline
