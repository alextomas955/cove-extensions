...
- CI (`.github/workflows/build.yml`) triggers on pull requests targeting `main`. It runs against
  every extension registered in `catalog.json` on every PR — there is no path filtering, so a
...
- The release workflow triggers independently, on tags matching `<extension-tagPrefix>v*` (e.g.
  `renamer/v1.0.0`), and only builds/releases the extension whose `catalog.json` entry the tag
  matches.
