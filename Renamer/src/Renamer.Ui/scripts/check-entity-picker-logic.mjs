#!/usr/bin/env node
/**
 * Offline gate for the pure entity-picker logic, cloned from check-options-roundtrip.mjs: no new
 * dependency, no test framework. entityPickerLogic.ts is a pure module with zero imports, so a
 ...
 * distinct env var (PICKER_LOGIC_MODULE) so the runners never collide. The scratch dir is removed on
...
const logicTs = path.join(uiRoot, "src", "entityPickerLogic.ts");
const testFile = path.join(here, "entity-picker-logic.test.mjs");

const outDir = mkdtempSync(path.join(tmpdir(), "rename-entity-picker-"));
