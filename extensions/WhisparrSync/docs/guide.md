...
This guide connects Cove to your Whisparr instance and finishes setup: a verified connection, the
defaults Whisparr applies when it adds an item, and a webhook Whisparr can call.
...
Click **Save** in the bar at the bottom. Your URL, version, and add defaults are stored. Your API
key is kept server-side — the field stays empty on reload and shows a "Key is set" pill; leaving it
blank on a later save keeps the stored key.
...
## Check that auto-import is working

There is no per-import log to page through. Two things tell you the pipeline is healthy, and both are
read-only:

- **The Import webhook section**, on the same settings page, carries the status line. It reads
  `Registered · last event {time}` once Whisparr has called it, or `Registered · no events received
  yet` if it never has. A recent last event is the proof that deliveries are arriving.
- **A banner appears when imports are failing.** If Whisparr imported a file at a path Cove could not
  open, the page names how many such imports have happened since the last successful one and shows the
  newest offending paths. A later success clears it. This is the case worth acting on — it usually
  means the two systems disagree about where the file lives, which is what path translation exists to
  fix.

To see what Whisparr itself is doing — what it is still looking for, downloading, or has already
grabbed — open [Wanted, queue &amp; history](./activity).
...
If a control is disabled and names a missing setting, open **Settings → Extensions → Whisparr Sync**, set
the setting it names, and save. The control becomes available. Nothing was sent to Whisparr while it was
refusing, so there is nothing to undo first.
...
If a menu item is disabled and names a missing setting, open **Settings → Extensions → Whisparr Sync**,
set the setting it names, and save; the item becomes available. This is different from an item reading "Currently available on Whisparr v3
(Eros)", which is about your Whisparr version and is not something a setting fixes.
