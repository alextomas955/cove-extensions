...
`/preview-sync` is `configure`-gated even though it only _reads_: it reaches the stored credentials to
...
thing?_ It never mutates Cove or Whisparr — `/preview-sync` opens an `AsNoTracking` read over the Cove
...
triggers a Whisparr search** — it only sets the flag; acquisition is Whisparr's own job.
...
**Loop-safety.** Every _add_ — per-scene add, add-all-missing, and owned-scene availability
