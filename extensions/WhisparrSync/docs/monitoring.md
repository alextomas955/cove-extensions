...
> Monitored in Whisparr · 1 of 147 in library

![A studio page in Cove with the Whisparr menu open from the action-row Whisparr button, showing Monitor in Whisparr checked, the monitor-scope choices, Add all missing, Search all monitored, and a "Monitored in Whisparr · X of Y in library" line at the foot of the menu.](/img/whisparr-sync/monitor-studio.png)
...
and grabs them using that entity's **own** root folder and quality profile in Whisparr — your
[Add defaults](./settings#add-defaults) if the extension created it, or whatever you already set in
Whisparr if the entity was there first. Turning monitoring on never triggers an immediate search — it
only sets the entity to monitored.
...
Whisparr import. The **Import webhook** section of the settings page shows when the last event arrived,
and the [Wanted, queue & history](./activity.md) page lists what Whisparr has already acquired.
...
- **New releases only** (the default) — you get **future** scenes as they appear, and the existing
  back-catalogue stays **visible but not wanted**, so Whisparr won't automatically grab it. Scenes you
  already own are left alone. You can still see what exists and grab any of it by hand.
- **All scenes** — **every** scene the entity offers is marked **wanted**, so Whisparr will acquire it
  as your indexers allow. That includes scenes you already own in Cove: Whisparr has no file of its
  own for them, so it counts them as missing and a later search can re-download them. Importing the
  result is idempotent, but the re-download still costs indexer traffic and disk. Choose **New releases
  only** if you don't want that.

Either scope, switching monitoring on never kicks off a search — you choose the scope, and Whisparr
acquires on its own schedule. On Whisparr v2 the same choice applies to the site's episodes.
...
Import and reconciliation keep working on v2 as well.
