...
// scene = episode).
//
// BOTH root at /data/media, which is the shared volume Cove also mounts. The extension requires Whisparr
// and Cove to see the library at the SAME path — that is the literal text of its pathNotVisible reason —
// so a root on Whisparr's own /config would make every On-Import round-trip unverifiable here no matter
// what else the harness did. v2's image defaults its media under /config; the root folder is
// configurable, and pointing it at the shared mount is what a real v2 user does too.
const VERSIONS = {
  v3: { image: 'ghcr.io/hotio/whisparr:v3', rootFolder: '/data/media' },
  v2: { image: 'ghcr.io/hotio/whisparr:v2', rootFolder: '/data/media' },
};
 ...
 * provisions a single root folder so the settings dropdowns and the add path have a real
...
export async function startWhisparr({ networkName, version = 'v3', metadataUrl, dataVolume } = {}) {
  ...
  // Provision a root folder so the settings dropdowns and the add path have a real target. The
