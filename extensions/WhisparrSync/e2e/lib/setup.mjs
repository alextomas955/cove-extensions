...
export async function startWhisparrSyncHarness({ version = "v3", pipeline = false } = {}) {
  ...
  try {
    ...
    const networks =
      typeof harness.container.getNetworkNames === "function"
        ? harness.container.getNetworkNames()
        : [];
    if (networks.length === 0) {
      throw new Error(
        "startWhisparrSyncHarness: could not resolve the harness Docker network from the Cove container",
      );
    }
    ...
    dataVolume = pipeline
      ? createPipelineVolume(String(harness.containerId).slice(0, 12))
      : undefined;
    ...
    if (pipeline) {
      ...
      provisioned = await provisionPipeline({
        whisparr,
        fakeIndexer,
        qbit,
