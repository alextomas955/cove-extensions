...
test("status projection reads grabbed-of-total over the real v2 site episode state", async () => {
  ...
  assert.equal(res.json.total, realEpisodes.length, "status total matches the real v2 episode count");
  assert.equal(res.json.grabbed, realGrabbed, "status grabbed matches the real v2 hasFile episode count");
