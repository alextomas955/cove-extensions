...
test("a real Whisparr v2 On-Import (series+episode) round-trips to Cove and is ingested", async () => {
  ...
  // /import-log carries two bounded aggregates rather than a per-attempt journal (the per-entry list was
  // dropped when the store was capped — nothing rendered it and a per-import row grows with the library),
  // so the v2 round-trip is proven by the import dependency reaching "imported" with no path mismatch.
  const log = await pollUntil(
    ...
    { timeoutMs: 120_000, intervalMs: 1000, label: "the v2 import dependency reaching the imported outcome" },
  );

  const importHealth = log.pipelineHealth.find((d) => d.dependency === "import");
  assert.equal(importHealth.outcome, "imported", "Cove ingested the episode Whisparr v2 reported importing");
  assert.equal(importHealth.consecutiveFailures, 0, "the v2 import leg records no failure");
  assert.equal(log.syncHealth.pathMismatch, 0, "no unresolved path mismatch on the v2 path");
  assert.ok(log.lastEventTicks > 0, "the inbound v2 webhook event was recorded");
});
...
test("the v2 downloadFolderImported history event reconciles against real Cove state", async () => {
  ...
  const importHealth = log.pipelineHealth.find((d) => d.dependency === "import");
  assert.ok(importHealth, "Cove's ingest state records the import leg the v2 history event references");
  assert.equal(
    importHealth.outcome,
    "imported",
    "the same real file reached Cove through the webhook channel that this history row exposes to the reconcile backstop",
  );
  assert.equal(log.syncHealth.pathMismatch, 0, "Cove opened that path — one import, two channels, no mismatch");
});
