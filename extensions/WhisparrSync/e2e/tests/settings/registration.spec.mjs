...
//
// Each generation's block starts only its own container, so neither execution pays for the other's
// boot, and each owns its installation: the connected generation is a global setting, and a shared
// one would be written out from under every sibling spec in the same worker.
 ...
 */
async function secondCoveHost(isolatedCove) {
  const { output } = await isolatedCove.exec(["sh", "-c", "hostname -i"]);
