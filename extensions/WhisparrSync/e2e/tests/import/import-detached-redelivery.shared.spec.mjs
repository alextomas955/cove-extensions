...
// The detached row and the host's answer to it are one implementation; what differs is the delivery
// document, the member the identity sits on, and the history the walk reads - each taken from the
// capture or the adapter for the connected generation.
//
...
const BANNER_ROUTE = extensionRoute("import/banner");

// How many rows the seeder puts on the instance before anything else, spanning every event type it
// declares.
const SEEDED_ROWS = 3;
...
async function bannerRoots(api) {
  const answered = await api.get(`${BANNER_ROUTE}?_=${randomUUID()}`);
  expect(answered.status, `GET ${BANNER_ROUTE} answered: ${answered.text.slice(0, 300)}`).toBe(200);
...
for (const generation of ["v3", "v2"]) {
  test.describe(`Whisparr ${generation}`, () => {
    ...
    test("a redelivery naming a path the extension detached answers inside its contract, and the backstop over the same state keeps its place", async ({
      ...
      const { adapter, api, whisparr } = connected;
      ...
      // Replace is what manufactures the detached row, so the whole scenario is about a state only
      // this behaviour reaches.
      const chosen = await api.put(SETTINGS_ROUTE, {
        selectedGeneration: connected.generation,
        v3: null,
        v2: null,
      ...
      expect(
        chosen.status,
        `saving the upgrade behaviour failed: ${chosen.text.slice(0, 300)}`,
      ).toBe(200);
      ...
      const { exitCode: firstStillThere } = await isolatedCove.exec(["test", "-f", first.covePath]);
      ...
      // Watched over a bounded window rather than polled: what is claimed is that nothing happened,
      // and a condition wait on an absence either returns at once or spends the whole budget.
      ...
      const markBefore = Date.parse(adapter.storedSection(marked).BackstopWatermarkUtc);
      ...
      const advanced = await pollUntil(
        () => readOptions(api),
        (options) =>
          Boolean(adapter.storedSection(options)?.BackstopWatermarkUtc) &&
          Date.parse(adapter.storedSection(options).BackstopWatermarkUtc) > markBefore,
        {
          timeoutMs: WATERMARK_BUDGET_MS,
          intervalMs: 2_000,
      ...
      expect(
        Date.parse(adapter.storedSection(advanced).BackstopWatermarkUtc),
