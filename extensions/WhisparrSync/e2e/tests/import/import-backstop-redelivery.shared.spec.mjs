...
// what arrived after the mark - is asserted by import-backstop.shared.spec.mjs. What is asserted
// here is what the SECOND arrival does: it joins the item the library already holds rather than
// standing a second one up beside it.
//
// One scenario, collected once per generation. The act turns on the library holding one item per
// scene, which is a fact about Cove rather than about a generation, and both generations carry a
// scene identity a second arrival can be matched on: v3 as a movie, v2 as an episode of a site.
//
// The passes below the last one are the arrangement rather than the subject. The wait ends on either
// settled outcome, so a duplicate reports as itself instead of as a poll timeout, and the file
// joining the item is what separates "did not duplicate" from "did nothing at all".
...
for (const generation of ["v3", "v2"]) {
  test.describe(`Whisparr ${generation}`, () => {
    ...
    test("a scene the library already holds, arriving again at a new path, joins the item rather than doubling it", async ({
      ...
      // Both seeded records hang off the instance's one library entry, so every arrival on this
      // channel names the same scene. Without the identity on the item there is nothing for the
      // second arrival to match on, which would make the claim below pass for the wrong reason.
