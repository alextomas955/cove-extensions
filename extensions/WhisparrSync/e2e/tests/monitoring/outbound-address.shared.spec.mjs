 ...
 *
 * Both cases take it on, so neither is decided by the setting rather than by the path. The whole
 * resource is read and written back with one member changed: these config routes REPLACE what they
 * are sent, so a body carrying only the one flag would blank the rest.
 */
async function linkIntoPlace(instance) {
  ...
  if (read.json?.[HARD_LINK_SETTING] !== true) {
    throw new Error(
      `linkIntoPlace: the instance reports ${HARD_LINK_SETTING}=${JSON.stringify(read.json?.[HARD_LINK_SETTING])}, so neither case below is about the path`,
    );
  }
}

/** Monitors the seeded studio from its own page, and waits for the instance's row to say so. */
async function monitorStudio(page, baseUrl, studio, studioMonitored) {
  const control = page
...
async function pressReflectOwned(page) {
...
async function followJob(api, jobId) {
