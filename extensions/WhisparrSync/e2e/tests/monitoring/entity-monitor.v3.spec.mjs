...
//
// The monitor and unmonitor gestures themselves are one scenario collected on both generations, in
// entity-monitor.shared.spec.mjs. The gesture is driven again here because the never-searched
...
// Configured per describe rather than inside the body: the body runs after its fixtures are built,
// so a budget raised there never covers the container stack this spec's fixtures start.
...
test("the control renders and works on both real detail pages, and the instance shows no search was started", async ({
  ...
  const { instance, studio: reachableStudio, studioMonitored, whisparr } = connected;
  ...
  const held = await studioMonitored();
  expect(
    held,
    `after the gesture the instance reports the studio as ${JSON.stringify(held)}, so the click changed nothing where it matters`,
  ).toBe(true);
