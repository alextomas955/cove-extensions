...
|------|---------|--------------|---------|
| `v3-stashdb.json` | v3 (eros) | StashDB UUID (`ForeignIds.StashId`) | Four: a `Tushy Raw` studio, a scene text-search, a `Lilly Bell` performer (per-performer add by StashDB id), and a single `Tushy Raw` scene (per-scene add by StashDB id) |
