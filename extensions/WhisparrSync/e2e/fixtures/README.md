...
├── media/
│   └── pipeline-media.mp4  # ~85KB synthetic MP4 the fake indexer serves over its webseed for the
│                           # acquire→import pipeline; long enough to clear Whisparr's sample-runtime
│                           # floor (see e2e/lib/setup.mjs). Seeded scenes use a different shared video.
