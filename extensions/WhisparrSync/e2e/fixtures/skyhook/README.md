...
| `GET /api/v3/movie/lookup?term=stash:{uuid}` | `GET /scene/{stashUuid}` | `v3-scene-tushy-raw.json` | one scene object |
...
| `POST /api/v3/performer {foreignId:{stashUuid}}` | `GET /performer/{stashUuid}` | `v3-performer-lilly-bell.json` | one performer object |
...
- v3 studio/site identity is a **StashDB UUID** carried in `ForeignIds.StashId`; scenes carry a numeric
  ...
  numeric TmdbId. The **studio**, **performer**, and **scene** entries are the StashDB-keyed identities:
  `v3-scene-tushy-raw.json` serves `/scene/019f3391-e257-7dd7-8c79-e399f2ad6ca5` (per-scene v3 add by
  StashDB id, `ForeignIds.TmdbId: 0`) and `v3-performer-lilly-bell.json` serves
  `/performer/7cf21be7-2a8e-44de-b19a-fa38aca6137c` (per-performer v3 add by StashDB id). Both are keyed
  in `index.json` and listed in `../allowlist/v3-stashdb.json`.
...
- **The one performer recording** (`v3-performer-lilly-bell.json`) → `Images`, `Tattoos` and `Piercings`
  emptied. Its `Aliases` are **kept** (unlike every other recording): they are spelling variants of the
  allowlisted performer's own SFW name, not third-party identities. Its remaining biographical fields
  (name, country, career years, measurements) are the real, SFW metadata the allowlist exception covers —
  a v3 performer add resolves on them. No explicit text and no image URL is committed.
