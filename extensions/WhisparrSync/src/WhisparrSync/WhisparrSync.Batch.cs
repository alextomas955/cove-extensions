...
/// <summary>
/// The bulk-action slice for the videos-list and studios/performers-list selections: the two endpoint
/// handlers, their Job-backed runners, and the pure per-op planning/dispatch helpers.
/// </summary>
public sealed partial class WhisparrSync
{
    // Videos/entities bulk routes: a configure-gated op over a capped id selection, each id resolved to a
    // scene/entity server-side.
    ...
    /// <summary>
    /// Registers the bulk-action slice's routes. Both POST {op, ids}, are configure-gated + stored-creds-only,
    /// and cap the selection before any per-item work; each delegates to its aggregator handler.
    /// </summary>
    private void MapBatchEndpoints(IEndpointRouteBuilder endpoints)
    {
        ...
        endpoints.MapPost(VideosBatchRoute,
            (VideosBatchRequest req, WhisparrClient client, CancellationToken ct)
                => VideosBatchAsync(req, client, ct)).ConfigureGated();

        endpoints.MapPost(EntitiesBatchRoute,
            (EntitiesBatchRequest req, WhisparrClient client, CancellationToken ct)
                => EntitiesBatchAsync(req, client, ct)).ConfigureGated();
    }
        ...
        VideosBatchRequest req, WhisparrClient client, CancellationToken ct)
    {
        ...
        // Above BOTH the inline branch and the enqueue below, so an incomplete configuration never becomes a job:
        // the Job Drawer must not show a run that was doomed before it started.
        ...
        if (_jobs is null)
        {
            ...
            {
                var result = await VideosBatchCoreAsync(op, coveIds, client, adapter, library, options, CapabilityPort(client), baseUrl, apiKey, ct);
                if (result.IsOk)
                {
        ...
        Cove.Core.Interfaces.IJobProgress progress, CancellationToken ct)
    {
        ...
        // A background job scope carries no request principal, so its ambient one is Anonymous — under which
        // CoveContext's authz filters skip every scene, so each LoadVideoByIdAsync below would return null and
        // the whole batch would report all-Skipped. The per-id resolution runs as System via the RunAsSystem seam.
    ...
    internal static async Task<WhisparrResult<VideosBatchResult>> VideosBatchCoreAsync(
        ...
        WhisparrOptions options, WhisparrCapabilityPort capability, string baseUrl, string apiKey, CancellationToken ct)
    {
        ...
        var actions = new SceneActions(client, options, library, capability);
        ...
        SceneActions actions, CancellationToken ct)
    {
        switch (op)
        {
            ...
            default:
                if (SceneStatusProjector.FindMovie(video.StashIds, movieIndex!) is not { } movie)
                {
    ...
    // result) is a Failed unit, never a false success. A leg whose only item was skipped is Skipped: the
    // user's own import exclusions declined it, which is not a failure of this unit.
    private static BatchUnitOutcome SucceededIfOne(WhisparrResult<BulkActionResult> result) => result switch
    {
        ...
        EntitiesBatchRequest req, WhisparrClient client, CancellationToken ct)
    {
        ...
        // Same position and reasoning as the videos batch: above both branches, so no job is created.
        if (RefuseIncompleteConfig(options) is { } refusal)
        {
            return refusal;
        }
        ...
        if (_jobs is null)
        {
            return await WithScopedLibraryAsync(options.StashDbEndpoint, options.TpdbEndpoint, async library =>
            {
                var result = await EntitiesBatchCoreAsync(kind, op, scope, coveIds, client, library, options, CapabilityPort(client), ct);
                LogBulkAction(kind, result.Total, result.Succeeded, result.Failed);
                return Results.Json(result, EnumStringResponseJsonOptions);
            });
        }
        ...
        WhisparrOptions options, Cove.Core.Interfaces.IJobProgress progress, CancellationToken ct)
    {
        ...
        // A background job scope carries no request principal, so its ambient one is Anonymous — under which
        // CoveContext's authz filters return no entity, so each per-entity identity/owned-scenes read would come
        // back empty and every unit would Skip with no outbound call. The fan-out runs as System via the seam.
        await RunAsSystemAsync(dbScope.ServiceProvider, async () =>
        {
            var batch = await _jobs!.RunBatchAsync(
                coveEntityIds,
                ...
                // drains — a StashDB-hammering refresh storm. Sequential pacing keeps it to
    ...
    internal static async Task<EntitiesBatchResult> EntitiesBatchCoreAsync(
        EntityKind kind, EntityBatchOp op, MonitorScope scope, IReadOnlyList<int> coveEntityIds,
        WhisparrClient client, ICoveLibraryPort library, WhisparrOptions options, WhisparrCapabilityPort capability,
    ...
    {
        EntityBatchOp.Monitor or EntityBatchOp.Unmonitor or EntityBatchOp.Search
            => kind == EntityKind.Studio || adapter is IWhisparrPerformerMonitor,
        EntityBatchOp.AddMissing => adapter is IWhisparrScenePush,
        EntityBatchOp.ReflectOwned => adapter is IWhisparrOwnedImport,
        _ => false,
    };
