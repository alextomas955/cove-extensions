...
internal sealed class WhisparrClient(HttpClient http)
{
    ...
    // What the whole-call budget above cannot express: a whole-library body legitimately takes longer than any
    // fixed wall, so the thing worth bounding there is a STALL — how long one read may go without returning —
    // and not the transfer. Raising CallTimeout instead would fail an honest large transfer to catch a dead one.
    private static readonly TimeSpan IdleReadTimeout = TimeSpan.FromSeconds(30);
    ...
    /// <summary>
    /// Reads <c>GET {baseUrl}/api/v3/movie</c> — the full movie set, unpaged (issue #218), for reconciliation
    ...
    /// <summary>
    /// Reads one page of <c>GET {baseUrl}/api/v3/history</c> newest-first — the reconcile backstop's data
    /// source (idempotent; bounded retry). The caller pages until it reaches the stored checkpoint, so a full
    /// history is never pulled at once.
    /// </summary>
    ...
    /// <summary>
    /// Reads one page of <c>GET {baseUrl}/api/v3/queue</c> — the live download queue (idempotent; bounded
    /// retry). <c>includeMovie=true</c> embeds the scene row so the projection resolves the studio/title
    /// without a second call. Both Whisparr generations serve this path (v2 is Sonarr-shaped); the projection
    /// normalizes either row.
    /// </summary>
    internal Task<WhisparrResult<WhisparrQueuePage>> ListQueueAsync(
        string baseUrl, string apiKey, int page, int pageSize, CancellationToken ct)
        => SendAsync(
            () => new HttpRequestMessage(
                HttpMethod.Get,
                $"{baseUrl.TrimEnd('/')}/api/v3/queue?page={page}&pageSize={pageSize}&includeMovie=true"),
            apiKey, GetMaxAttempts,
            (resp, token) => DeserializeAsync(resp, WhisparrJsonContext.Default.WhisparrQueuePage, token),
            ct);

    /// <summary>
    /// Reads one page of <c>GET {baseUrl}/api/v3/wanted/missing</c> — the live wanted set (monitored scenes
    /// with no file), newest-first (idempotent; bounded retry). <c>monitored=true</c> scopes it to the wanted
    /// definition. Both generations serve this path; v2 returns episode-shaped rows that bind into <see
    /// cref="WhisparrMovie"/>.
    /// </summary>
    internal Task<WhisparrResult<WhisparrMoviePage>> ListWantedMissingAsync(
        string baseUrl, string apiKey, int page, int pageSize, CancellationToken ct)
        => SendAsync(
            () => new HttpRequestMessage(
                HttpMethod.Get,
                $"{baseUrl.TrimEnd('/')}/api/v3/wanted/missing?page={page}&pageSize={pageSize}&sortKey=date&sortDirection=descending&monitored=true"),
            apiKey, GetMaxAttempts,
            (resp, token) => DeserializeAsync(resp, WhisparrJsonContext.Default.WhisparrMoviePage, token),
            ct);

    /// <summary>
    /// Reads <c>GET {baseUrl}/api/v3/series</c> — the Whisparr v2 studio/site list (idempotent; bounded retry).
    /// The v2 adapter walks these into scenes; v2 has no <c>/movie</c> entity.
    /// </summary>
    ...
    /// <c>seriesId</c> query is REQUIRED; idempotent, bounded retry).
    /// </summary>
    ...
    /// (the <c>seriesId</c> query is REQUIRED; idempotent, bounded retry). Source of the path leg.
    /// </summary>
    ...
    internal Task<WhisparrResult<WhisparrManualImportItem[]>> ListManualImportAsync(
        ...
        => RefuseBlank<WhisparrManualImportItem[]>(folder, nameof(folder)) is { } refusal
    ...
    internal Task<WhisparrResult<WhisparrSeries[]>> LookupSeriesAsync(string baseUrl, string apiKey, string term, CancellationToken ct)
        => RefuseBlank<WhisparrSeries[]>(term, nameof(term)) is { } refusal
        ? Task.FromResult(refusal)
        : SendAsync(
            () => new HttpRequestMessage(
                HttpMethod.Get,
    ...
    internal Task<WhisparrResult<WhisparrStudio[]>> GetStudioByStashIdAsync(string baseUrl, string apiKey, string stashId, CancellationToken ct)
        => RefuseBlank<WhisparrStudio[]>(stashId, nameof(stashId)) is { } refusal
        ? Task.FromResult(refusal)
        : SendAsync(
            () => new HttpRequestMessage(
                HttpMethod.Get,
    ...
    internal Task<WhisparrResult<WhisparrPerformer>> GetPerformerByStashIdAsync(string baseUrl, string apiKey, string stashId, CancellationToken ct)
        => RefuseBlank<WhisparrPerformer>(stashId, nameof(stashId)) is { } refusal
        ? Task.FromResult(refusal)
        : SendAsync(
            () => new HttpRequestMessage(
                HttpMethod.Get,
    ...
    /// <summary>
    /// Reads <c>GET {baseUrl}/api/v3/release?movieId={movieId}</c> — the on-demand indexer release list for one
    /// movie (idempotent, bounded retry). Only the release count is consumed by the caller. NOT invoked eagerly
    /// — the scene panel calls it only on an explicit user expand, one movieId at a time.
    /// </summary>
    ...
    internal Task<WhisparrResult<WhisparrMovie[]>> GetMovieByStashIdAsync(string baseUrl, string apiKey, string stashId, CancellationToken ct)
        => RefuseBlank<WhisparrMovie[]>(stashId, nameof(stashId)) is { } refusal
        ? Task.FromResult(refusal)
        : SendAsync(
            () => new HttpRequestMessage(
                HttpMethod.Get,
    ...
    /// An add whose studio the user's import exclusions cover classifies
    /// <see cref="WhisparrResultState.Excluded"/> — a skip the caller reports, never a failure.
    /// </remarks>
    internal Task<WhisparrResult<WhisparrMovie>> CreateMovieAsync(string baseUrl, string apiKey, string movieJson, CancellationToken ct)
        => SendAsync(
            ...
            excludedBodyMatch: IsExcludedByConfigBody);
    ...
    // Whisparr Eros throws ExcludedException from AddMovieService.SetPropertiesAndValidate when the scene's
    // studio is on the import-exclusion list, whatever `monitored` the add asks for. Measured against 3.3.4:
    // HTTP 500 + { "message": "Studio: [X] has been excluded",
    // "description": "NzbDrone.Core.Exceptions.ExcludedException: …" }.
    //
    // The refusal is NOT idempotent: the failed add leaves a scene-level exclusion row behind, and a second
    // identical add consumes that row, skips the studio check and SUCCEEDS. So a re-run of a bulk add will
    // add scenes this classified as skipped the first time — a Whisparr-side behaviour this cannot mask.
    //
    // Matching the BODY rather than the status is the deliberate trade-off: 500 is also Servarr's generic
    // fault code, so keying on it would swallow real faults, and the status a future build answers for a
    // refused add is far less settled than the exception itself. `description` names the exception type and
    // is the stronger marker, but a build that suppresses stack detail omits it — hence the message clause
    // as fallback, which is the wording shared by the studio, after-date, performer and tag exclusion
    // throws. Both markers are English literals — a localized Whisparr is not recognised here.
    private static bool IsExcludedByConfigBody(string body)
        => body.Contains("ExcludedException", StringComparison.Ordinal)
            || body.Contains("has been excluded", StringComparison.OrdinalIgnoreCase);
    ...
    /// <summary>
    /// Reads <c>GET {baseUrl}/api/v3/command/{commandId}</c> — one queued command's status (idempotent; bounded
    /// retry), the read the wait loop polls until a refresh reports <c>completed</c>. <paramref
    /// name="commandId"/> is a Whisparr-supplied id (from <see cref="SendCommandForIdAsync"/>) used only to
    /// address that same command. Transport-only: the classify-not-throw guards are inherited from <see
    /// cref="SendAsync"/>; the adapter interprets the status string.
    /// </summary>
    ...
    /// <summary>
    /// The most movie ids one <see cref="GetMoviesByIdsAsync"/> call may carry.
    /// </summary>
    /// <remarks>
    /// It bounds the PER-CALL payload — at most this many ids in one request body and this many rows in one
    /// response, roughly 1.7 MB at the row size recorded in <c>e2e/fixtures/wire/movie-row.json</c>. It does NOT
    /// bound an entity's total catalogue, which is bounded by the entity; the caller chunks until its id list is
    /// exhausted. Distinct from <c>MaxEntityIdsPerRequest</c>, which limits what a caller may ask this extension
    /// for rather than what this extension asks Whisparr for in one go.
    /// </remarks>
    internal const int MaxMovieIdsPerBulkRead = 1000;

    /// <summary>
    /// Reads <c>GET {baseUrl}/api/v3/movie/listbystudioforeignid?studioForeignId={foreignId}</c> — the movie
    /// ids attributed to one studio (idempotent; bounded retry).
    /// </summary>
    /// <remarks>
    /// Answers an array of integer movie IDS, not rows, so the caller hydrates through
    /// <see cref="GetMoviesByIdsAsync"/>. An empty array does NOT mean the studio is unknown — this route
    /// answers identically for an unknown studio and a known one holding nothing
    /// (<c>e2e/fixtures/wire/sibling-endpoints.json</c>); <see cref="GetStudioByForeignIdAsync"/> is what
    /// separates them.
    /// </remarks>
    internal Task<WhisparrResult<int[]>> ListMovieIdsByStudioForeignIdAsync(
        string baseUrl, string apiKey, string foreignId, CancellationToken ct)
        => RefuseBlank<int[]>(foreignId, nameof(foreignId)) is { } refusal
        ? Task.FromResult(refusal)
        : SendAsync(
            () => new HttpRequestMessage(
                HttpMethod.Get,
                $"{baseUrl.TrimEnd('/')}/api/v3/movie/listbystudioforeignid?studioForeignId={Uri.EscapeDataString(foreignId)}"),
            apiKey, GetMaxAttempts,
            (resp, token) => DeserializeAsync(resp, WhisparrJsonContext.Default.Int32Array, token),
            ct);

    /// <summary>
    /// Reads <c>GET {baseUrl}/api/v3/movie/listbyperformerforeignid?performerForeignId={foreignId}</c> — the
    /// movie ids attributed to one performer (idempotent; bounded retry). Same id-only answer and same
    /// empty-is-ambiguous caveat as <see cref="ListMovieIdsByStudioForeignIdAsync"/>.
    /// </summary>
    internal Task<WhisparrResult<int[]>> ListMovieIdsByPerformerForeignIdAsync(
        string baseUrl, string apiKey, string foreignId, CancellationToken ct)
        => RefuseBlank<int[]>(foreignId, nameof(foreignId)) is { } refusal
        ? Task.FromResult(refusal)
        : SendAsync(
            () => new HttpRequestMessage(
                HttpMethod.Get,
                $"{baseUrl.TrimEnd('/')}/api/v3/movie/listbyperformerforeignid?performerForeignId={Uri.EscapeDataString(foreignId)}"),
            apiKey, GetMaxAttempts,
            (resp, token) => DeserializeAsync(resp, WhisparrJsonContext.Default.Int32Array, token),
            ct);

    /// <summary>
    /// Reads <c>GET {baseUrl}/api/v3/studio/{foreignId}</c> — the studio addressed by its StashDB id
    /// (path param; idempotent, bounded retry). An HTTP 404 OR 500 classifies
    /// <see cref="WhisparrResultState.Absent"/>, mirroring <see cref="GetPerformerByStashIdAsync"/>.
    /// </summary>
    /// <remarks>
    /// Distinct from <see cref="GetStudioByStashIdAsync"/>, which queries the collection and answers a possibly
    /// empty array. This one exists for its 404: it is the recorded discriminator between a studio Whisparr does
    /// not know and one it knows that holds no movies.
    /// </remarks>
    internal Task<WhisparrResult<WhisparrStudio>> GetStudioByForeignIdAsync(
        string baseUrl, string apiKey, string foreignId, CancellationToken ct)
        => RefuseBlank<WhisparrStudio>(foreignId, nameof(foreignId)) is { } refusal
        ? Task.FromResult(refusal)
        : SendAsync(
            () => new HttpRequestMessage(
                HttpMethod.Get,
                $"{baseUrl.TrimEnd('/')}/api/v3/studio/{Uri.EscapeDataString(foreignId)}"),
            apiKey, GetMaxAttempts,
            (resp, token) => DeserializeAsync(resp, WhisparrJsonContext.Default.WhisparrStudio, token),
            ct,
            absentCodes: AbsentGetCodes);
    ...
    /// <summary>
    /// Posts <paramref name="ids"/> to <c>POST {baseUrl}/api/v3/movie/bulk</c> and returns those movies —
    /// ONE request for ONE chunk; the chunking loop belongs to the caller.
    /// </summary>
    /// <remarks>
    /// A POST that READS. Verified live against 3.3.4.794 by reading the movie set before and after the call
    /// (8 rows, unchanged) rather than by trusting its 200 — this route hydrates ids and adds nothing.
    /// </remarks>
    internal Task<WhisparrResult<WhisparrMovie[]>> GetMoviesByIdsAsync(
        string baseUrl, string apiKey, IReadOnlyList<int> ids, CancellationToken ct)
        => SendAsync(
            () => new HttpRequestMessage(HttpMethod.Post, $"{baseUrl.TrimEnd('/')}/api/v3/movie/bulk")
            {
                Content = new StringContent(
                    JsonSerializer.Serialize(ids, WhisparrJsonContext.Default.IReadOnlyListInt32),
                    Encoding.UTF8,
                    "application/json"),
            },
            apiKey, GetMaxAttempts,
    ...
    /// <summary>
    /// Reads <c>GET {baseUrl}/docs/v3/openapi.json</c> — the instance's own generated API description
    /// (idempotent; bounded retry).
    /// </summary>
    /// <remarks>
    /// No API key is sent: the route answers without one (read live off 3.3.4.794). It still goes through the
    /// shared send loop so a reverse-proxy HTML page or a 502 classifies as NotWhisparr rather than binding
    /// garbage into a capability decision.
    /// </remarks>
    internal Task<WhisparrResult<OpenApiDocument>> GetOpenApiDocumentAsync(string baseUrl, CancellationToken ct)
        => SendAsync(
            () => new HttpRequestMessage(HttpMethod.Get, $"{baseUrl.TrimEnd('/')}/docs/v3/openapi.json"),
            string.Empty, GetMaxAttempts,
            (resp, token) => DeserializeAsync(resp, WhisparrJsonContext.Default.OpenApiDocument, token),
            ct);

    /// <summary>
    /// The refusal every read applies to a caller value it would interpolate into a filter or a path
    /// segment: <c>null</c> when the value is usable, otherwise the result to return unsent.
    /// </summary>
    /// <remarks>
    /// A blank value is not a narrow read, it is the widest one Whisparr offers, by two different
    /// mechanisms: an empty query filter is read as NO filter and answers the entire set, and an empty path
    /// segment collapses the URL onto the collection route. Both cost the whole library, so the request is
    /// refused before it exists rather than sent and discarded.
    /// </remarks>
    private static WhisparrResult<T>? RefuseBlank<T>(string value, string parameterName)
        => string.IsNullOrWhiteSpace(value)
            ? WhisparrResult<T>.NotAsked($"'{parameterName}' was blank, so Whisparr was not asked")
            : null;
    ...
    private async Task<WhisparrResult<T>> SendAsync<T>(
        ...
        Func<string, bool>? excludedBodyMatch = null,
        ...
        HttpCompletionOption completionOption = HttpCompletionOption.ResponseContentRead)
    {
        ...
        for (var attempt = 1; attempt <= maxAttempts; attempt++)
        {
            ...
            try
            {
                ...
                // Guard the target URI at the transport edge: a malformed absolute URL throws
                ...
                using var resp = await http.SendAsync(req, completionOption, linked.Token);
                ...
                if (!resp.IsSuccessStatusCode)
                {
                    ...
                    // A refusal the caller's own Whisparr configuration produced is a data outcome, not a
                    // transport fault.
                    if (excludedBodyMatch is not null && excludedBodyMatch(errorBody))
                    {
                        return WhisparrResult<T>.Excluded(message ?? "excluded in Whisparr");
                    }
                ...
                // The terminal body read gets a token that is NOT the send budget's, and only here. Under
                // headers-only completion the transport drops its send-token registration before the response
                // stream exists, so handing the send token to this callback would silently restore the
                // whole-call wall over the one transfer the wall must not govern — with every test still
                // green. That body is bounded by IdleReadTimeout instead. Every other callback site above
                // keeps the send token: those bodies are small and their semantics must not move.
                var bodyToken = completionOption == HttpCompletionOption.ResponseHeadersRead ? ct : linked.Token;
                return await onSuccess(resp, bodyToken);
            }
            ...
            {
                // Once the body is read from the response stream rather than inside the send call, a
                // mid-transfer failure arrives as an IOException-derived type — HttpIOException derives from
                // IOException, NOT from HttpRequestException — and the catch above never sees it. Uncaught it
                // escapes the classify-not-throw boundary as a 500. The stall budget reports through the same
                // catch, for the same reason: both are the transfer dying part-way.
                last = WhisparrResult<T>.Unreachable(ex.Message);
            }
    ...
    /// <summary>
    /// Folds a JSON array response element by element, without ever materialising the array.
    /// </summary>
    /// <remarks>
    /// <para>
    /// The enumeration runs INSIDE <see cref="SendAsync"/>'s <c>try</c>/<c>using</c>, which is why this is a
    /// fold rather than a returned enumerable: a caller handed an <c>IAsyncEnumerable</c> would enumerate a
    /// disposed response, a parse failure at element four hundred thousand could not be classified back into
    /// a result already returned Ok, and the bounded retry could not re-issue a request whose partial output
    /// the caller had consumed.
    /// </para>
    /// <para>
    /// <paramref name="seed"/> is a factory and not a value because the retry loop re-invokes the request
    /// factory per attempt. A shared accumulator would fold the same rows twice and answer plausibly wrong.
    /// </para>
    /// </remarks>
    private static async Task<WhisparrResult<TAccumulator>> FoldStreamAsync<TElement, TAccumulator>(
        HttpResponseMessage resp,
        JsonTypeInfo<TElement> elementInfo,
        Func<TAccumulator> seed,
        Func<TAccumulator, TElement, TAccumulator> fold,
        ...
        CancellationToken ct)
    {
        try
        {
            ...
            var content = await resp.Content.ReadAsStreamAsync(ct);

            // The HttpContent extension offers no seam for the stall budget, so the stream overload is what
            // this takes — and it binds the ELEMENT type info, never the array's.
            await using var bounded = new IdleReadTimeoutStream(content, idleReadTimeout);
            await foreach (var element in JsonSerializer.DeserializeAsyncEnumerable(bounded, elementInfo, ct))
            {
        ...
        {
            return WhisparrResult<TAccumulator>.NotWhisparr();
        }
    }

    /// <summary>
    /// Reads <c>GET {baseUrl}/api/v3/movie?excludeLocalCovers=true</c> and folds it into
    /// <typeparamref name="TAccumulator"/> as it arrives — the whole movie set answered without the row array
    /// ever existing (idempotent; bounded retry).
    /// </summary>
    /// <remarks>
    /// <para>
    /// Each row binds as <see cref="WhisparrMovieFacts"/>: an unmapped JSON member costs nothing to skip, so
    /// the narrow projection retains a small fraction of the bytes for the same answer. Callers needing the
    /// nested quality name or the cutoff flag read the full row through <see cref="ListMoviesAsync"/>.
    /// </para>
    /// <para>
    /// <c>excludeLocalCovers</c> is baked in rather than exposed, because a caller that can turn a pure
    /// payload reduction off is a caller that will. It was measured honoured on 3.3.4.794 — same row set,
    /// fewer bytes, and what disappears is each image entry's locally-cached path
    /// (<c>e2e/fixtures/wire/movie-list-probe.json</c>). It is applied HERE only: the discovery projection
    /// falls back to that cached path when a row carries no source-served url, so a row Whisparr has a cover
    /// for and the source does not would silently lose its image if the shared read were narrowed too. This
    /// caller reads no image at all.
    /// </para>
    /// </remarks>
    internal Task<WhisparrResult<TAccumulator>> FoldMovieFactsAsync<TAccumulator>(
        string baseUrl, string apiKey, Func<TAccumulator> seed,
        Func<TAccumulator, WhisparrMovieFacts, TAccumulator> fold, CancellationToken ct)
        => SendAsync(
            () => new HttpRequestMessage(
                HttpMethod.Get,
                $"{baseUrl.TrimEnd('/')}/api/v3/movie?excludeLocalCovers=true"),
            apiKey, GetMaxAttempts,
            (resp, token) => FoldStreamAsync(
                resp, WhisparrJsonContext.Default.WhisparrMovieFacts, seed, fold, IdleReadTimeout, token),
            ct,
