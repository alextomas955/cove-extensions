    ...
    Func<int, CancellationToken, Task<WhisparrResult<WhisparrHistoryPage>>> listHistoryPage)
{
    ...
    private async Task IngestRecordAsync(WhisparrHistoryRecord record, EventLedger ledger, ImportLog log, CancellationToken ct)
    {
        ...
        try
        {
            // identity: null — the backstop's history row carries only the Whisparr movie id, not the scene's
            // StashDB/TPDB id, so it does not enrich here; the On-Import webhook (which carries the id) is the
            // enrichment channel. Resolving the id from the movie on this path is a follow-up (see Phase 27).
        ...
        await log.AppendAsync(
            new ImportLogEntry(
                ...
                outcome.Kind, outcome.CoveEntityId, result, outcome.Reason, key),
            ct);
