...
/// <summary>
/// The polling-reconcile backstop: it pages Whisparr history newest-first since a stored
    ...
    Action<string>? onHealthRecordFailure = null)
{
    ...
    // The single named import eventType (Radarr-family; kept one const so the live-confirm is
    ...
    // A hard page cap so a mis-sorted / unbounded history can never spin the job forever (a DoS guard).
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        // those import events (defeating the "never webhook-only" backstop). Keeping the checkpoint
        ...
        if (completed && highWater != current.LastRecordId)
        {
            ...
            // Every hist:{id} at/below the new checkpoint is now provably dead — a later pass stops before
            // reaching it — so prune them to bound the ledger. The cross-channel ImportKeys are left in place
            // (their own window cap handles them); this only sheds the poll-self keys the checkpoint superseded.
            await ledger.PruneHistBelowAsync(highWater, ct);
        }
    }
    ...
    private async Task SeedAsync(Checkpoint checkpoint, CancellationToken ct)
    {
        ...
        if (!first.IsOk || first.Value is null)
        {
            ...
            // next successful tick retro-ingest the ENTIRE prior library. Seed nothing, ingest nothing.
            return;
        }
        ...
        int lastRecordId, CancellationToken ct)
    {
        ...
        for (var page = 1; page <= MaxPages; page++)
        {
            ...
            if (!result.IsOk)
            {
                ...
                // Report NOT completed so RunAsync keeps the checkpoint and re-fetches the gap next run.
        ...
        // Fell out of the loop by hitting the MaxPages cap (a DoS guard) WITHOUT reaching the checkpoint:
    ...
    private async Task IngestRecordAsync(WhisparrHistoryRecord record, EventLedger ledger, ImportLog log, CancellationToken ct)
    {
        ...
        // The loser lost the race — the webhook already claimed it — so it skips without a second entity.
        if (!await ledger.TryClaimAsync(key, ct))
        {
            return; // already ingested via the webhook or a prior poll — a duplicate no-op
        }

        IngestOutcome outcome;
        try
        {
            // identity is null here: the backstop's history row carries only the Whisparr movie id, not the
            // scene's StashDB/TPDB id — enriching with that id is the On-Import webhook's job.
            outcome = await coordinator.IngestAsync(path, existingId: null, identity: null, ct);
        }
        catch
        {
            // An unexpected ingest fault must not permanently swallow the import: release the shared claim so a
            // later poll (or the webhook) re-processes it rather than skipping it forever.
            await ledger.ReleaseAsync(key, ct);
            throw;
        }

        // fromWebhook: false — the poll never stamps the "last event" line (that is a webhook-only signal),
        // but a poll success still clears the sync-health window and a poll path-mismatch still flags it.
        var result = outcome.Result == IngestResult.Imported ? "Imported" : "Flagged";
        await log.RecordOutcomeAsync(false, DateTime.UtcNow.Ticks, result, outcome.Reason, path, ct);
        await HealthStore.TryRecordAsync(
            store,
            HealthDependency.Import,
            HealthOutcome.FromImport(result, outcome.Reason),
            reason => onHealthRecordFailure?.Invoke(reason),
            ct);
