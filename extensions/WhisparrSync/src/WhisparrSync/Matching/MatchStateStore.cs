...
/// <summary>
/// Persists the Cove↔Whisparr match map as a SINGLE JSON-array blob under one
...
/// so the whole map lives in ONE blob rather than per-entry keys (acceptable at the
...
internal sealed class MatchStateStore(IExtensionStore store)
{
    ...
    private static MatchState[] Parse(string? json)
    {
        ...
        {
            return []; // corrupt/hand-edited blob → empty map, never throws
        }
    }
