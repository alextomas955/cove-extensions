...
public sealed partial class WhisparrSync
{
    ...
    // The batch's state map serializes SceneWhisparrState as a DICTIONARY VALUE — which carries no property-level
    // [JsonConverter], and a plain options-level JsonStringEnumConverter would win over the enum's type-level
    // attribute and emit PascalCase (the pitfall the enum's own docs warn about). So this endpoint's options
    // register SceneWhisparrStateJsonConverter directly, pinning the same camelCase wire strings the scene
    // panel / summary already emit — the frontend status store keys on those exact strings.
        ...
        [property: JsonConverter(typeof(SceneWhisparrStateJsonConverter))] SceneWhisparrState WhisparrState)
    {
        public static ReconRow From(MatchResult r, string status, IReadOnlySet<string> excludedSet) => new(
            ...
            Status: status,
            // A recon row is a present Whisparr movie (always "added"): the projector yields
            // downloaded/monitored/unmonitored, or excluded when the movie's StashId is in the exclusion read.
            // The camelCase wire string is pinned by SceneWhisparrState's converter.
