...
public sealed record WhisparrOptions
{
    ...
    /// <see cref="BaseUrl"/>/<see cref="ApiKey"/>/<see cref="RootFolderId"/>/<see cref="QualityProfileId"/> remain
        ...
        MonitorScope? defaultMonitorScope = null)
    {
        ...
        var connection = new WhisparrConnection(effectiveUrl, effectiveKey, rootFolderId, qualityProfileId);
...
/// <summary>
/// One saved Whisparr connection — the URL, API key, and the instance-specific root-folder / quality-profile
/// ids for one version. Held per version in <see cref="WhisparrOptions.SavedConnections"/> so switching versions
...
/// </summary>
public sealed record WhisparrConnection(string BaseUrl, string ApiKey, int RootFolderId, int QualityProfileId);
    ...
    public static OptionsView From(WhisparrOptions options)
    {
        ...
        if (!connections.ContainsKey(options.SelectedVersion))
        {
            connections[options.SelectedVersion] = new ConnectionView(
                options.BaseUrl, options.RootFolderId, options.QualityProfileId,
...
/// <summary>One saved connection as the UI sees it: the URL + root/quality ids + a <c>hasApiKey</c> flag; the raw
/// key is never projected. The UI shows the URL/root/profile on a version toggle and a "key is set" badge.</summary>
    ...
    [property: JsonPropertyName("hasApiKey")] bool HasApiKey)
{
    public static ConnectionView From(WhisparrConnection connection)
        => new(connection.BaseUrl, connection.RootFolderId, connection.QualityProfileId,
