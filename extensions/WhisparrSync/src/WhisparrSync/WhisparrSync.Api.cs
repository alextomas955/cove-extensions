...
public sealed partial class WhisparrSync
{
    ...
    // The v2 owned-scene import: attach files Cove already owns to their matching fileless v2 scenes IN PLACE
    // (targeted ManualImport, no move/copy/grab). Same posture as bulk-add-missing: configure-gated +
    // stored-creds-only, entity resolved SERVER-SIDE from the forwarded kind + Cove id. v2-only — v3 defers
    // (VERSION_UNSUPPORTED 400), owned-import stays deferred there this release.
    ...
    internal UIManifest BuildManifest(string? selectedVersion)
    {
        ...
        if (!string.Equals(selectedVersion, "v2", StringComparison.OrdinalIgnoreCase))
        {
            builder
                ...
                .AddTab(pageType: "video", key: "whisparr", label: "Whisparr", componentName: "WhisparrScenePanel", icon: "Radar")
