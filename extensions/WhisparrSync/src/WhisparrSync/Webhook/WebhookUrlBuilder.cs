...
/// <summary>
/// Mints the webhook secret and builds the copy-paste webhook URL Whisparr posts events to. The
/// secret is a high-entropy token generated with <see cref="System.Security.Cryptography.RandomNumberGenerator"/> (never
/// <c>System.Random</c>) and persisted once in <c>WhisparrOptions.WebhookSecret</c> so the
/// URL is stable across calls. This builder only generates and (best-effort) registers the URL; a separate
/// receiver endpoint consumes it.
/// </summary>
internal static class WebhookUrlBuilder
{
    /// <summary>The extension's inbound webhook path.</summary>
