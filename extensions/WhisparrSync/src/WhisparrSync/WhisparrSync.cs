...
public sealed partial class WhisparrSync : FullExtensionBase, IDisposable
{
    ...
    // Baselines on the Cove 1.1.0 release, which is where extension pages, tabs and APIs became
    // permission- and auth-checked by the host. (The host APIs it binds to — the metadata-server
    ...
    public override void ConfigureServices(IServiceCollection services, ExtensionContext context)
    {
        ...
        // The direct-metadata sources ride their own pooled clients beside the Whisparr one: StashDB (v3, stash-box
        // GraphQL) and ThePornDB (v2, REST — its stash-box queryScenes is gated, verified live).
        services.AddHttpClient<StashDbGraphQlClient>();
        services.AddHttpClient<TpdbClient>();
    }
    ...
    private async Task RunReconcileLoopAsync(CancellationToken ct)
    {
        try
        {
            var scheduler = new ReconcileScheduler(
                _jobs,
                (_, c) => RunReconcileAsync(c),
                LogReconcileEnqueueFault,
                TimeSpan.FromMinutes(DefaultReconcileIntervalMinutes));
