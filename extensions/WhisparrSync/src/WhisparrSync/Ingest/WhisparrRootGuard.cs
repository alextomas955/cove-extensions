...
internal static class WhisparrRootGuard
{
    ...
    /// to follow symlinks) and normalized with the shared <see cref="EventLedger.NormalizePath"/>, then
    ...
    private static bool TryCanonicalize(string value, out string canonical)
    {
        try
        {
            ...
            // resolve the real on-disk path so a symlink inside a root whose target ESCAPES the root is
            // caught. NormalizePath (separator-unify + trailing-trim, CASE-SENSITIVE) makes the prefix
    ...
    // ACTUALLY lands. A not-yet-existing path has no link to follow, so its lexical form stands (the
