...
/// — / <see cref="IsUpgrade"/> / <see cref="DownloadId"/> are load-bearing here.
...
internal sealed record WebhookMovie
{
    ...
    // The scene's StashDB id — the same id a Cove scene carries after Identify, so the ingested video can be
    // stamped + identified by it (v3 keys outward on StashDB; see SceneEnricher).
...
internal sealed record WebhookSeries
{
    ...
    // v2 stores the site's ThePornDB id in the Sonarr tvdbId slot — the id a Cove studio carries, used to
    // stamp/identify the ingested v2 scene against ThePornDB (see SceneEnricher).
