...
/// webhooks ONLY: a secret in a URL query is routinely recorded by proxy/access logs, so when it is
...
internal sealed class WebhookReceiver(
    ...
    Action? onQueryTokenFallback = null,
    Action<string>? onHealthRecordFailure = null)
{
    ...
    {
        // Prefer the X-Cove-Token header; the ?token= query is a fallback for hand-pasted webhooks only.
        ...
        {
            // One condition, two opposite meanings, and only one of them is a fact worth remembering. With NO
            // secret minted this route is reachable by any anonymous caller, so recording here would let a
            // stranger plant a sticky import-channel failure on a fresh install before the user has opened the
            // settings page — and nothing has actually been rejected, because nothing could have been accepted.
            // A mismatch against a CONFIGURED secret is a genuine rejection, and the only one here worth a
            // health record.
            if (expected.Length != 0)
            {
                await RecordImportHealthAsync(HealthOutcome.WebhookRejected(), ct);
            }

            return Results.Json(new WebhookAckResponse("UNAUTHORIZED"), ResponseJsonOptions, statusCode: 401);
        }
        ...
        if (payload is null)
        {
            return Results.Json(new WebhookAckResponse("OK"), ResponseJsonOptions, statusCode: 200);
        }
    ...
    private async Task HandleDownloadAsync(WebhookPayload payload, CancellationToken ct)
    {
        ...
        {
            // A duplicate delivery is still a webhook event (it stamps the "last event" line) but no import
            // outcome — a Skipped is neither a success nor a path-mismatch, so it only advances the event tick.
            await log.RecordOutcomeAsync(true, DateTime.UtcNow.Ticks, "Skipped", "duplicate delivery", path, ct);
            await RecordImportHealthAsync(HealthOutcome.FromImport("Skipped", "duplicate delivery"), ct);
            return;
        }

        // No upgrade-in-place handle in this build: an upgrade re-import takes the create path, which the ledger
        // dedups on replay and the host's unique (ParentFolderId, Basename) index makes structurally safe. A
        // durable WhisparrMovieId → Cove id map is the wrong shape for it — it grows with the library — so the
        // bounded form is a remote-id lookup against Cove at event time.
        int? existingId = null;
        ...
        // The claim already recorded the key. The event is folded into the status record once (imported OR
        // flagged), so a Whisparr retry or the reconcile-poll overlap is a duplicate no-op; the reconcile
        // backstop keys the same ImportKey, so it will not re-process it.
        var result = outcome.Result == IngestResult.Imported ? "Imported" : "Flagged";
        await log.RecordOutcomeAsync(true, DateTime.UtcNow.Ticks, result, outcome.Reason, path, ct);
        await RecordImportHealthAsync(HealthOutcome.FromImport(result, outcome.Reason), ct);
    }

    // The health write is best-effort so a store hiccup cannot break an ingest, and the failure callback (wired
    // by the route registration) turns that into exactly one structured log line rather than a silent swallow.
    private Task RecordImportHealthAsync(HealthObservation observation, CancellationToken ct)
        => HealthStore.TryRecordAsync(
            store, HealthDependency.Import, observation, reason => onHealthRecordFailure?.Invoke(reason), ct);
