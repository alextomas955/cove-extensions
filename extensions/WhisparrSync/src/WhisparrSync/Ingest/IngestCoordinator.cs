    ...
    Func<CancellationToken, ValueTask<IReadOnlyList<string>>> allowedRootsProvider)
{
    ...
    public async Task<IngestOutcome> IngestAsync(string path, int? existingId, SceneIdentity? identity, CancellationToken ct)
    {
        ...
        // A Whisparr On-Import carries no Cove principal, so the ambient one is Anonymous — under which
        // CoveContext's per-principal authz filters return ZERO rows, leaving the enrichment's video lookup
        // empty and the scene un-stamped. This is a trusted host-triggered library op, so its whole span
        // (import, enrich, scan) runs as System via the RunAsSystem seam.
        return await RunAsSystemAsync(scope.ServiceProvider, async () =>
        {
            try
            {
                ...
                if (identity is { } id)
                {
                    // The DbContext is absent in a host with no DB scope (and in the coordinator's own unit
                    // tests); enrichment is skipped there — the import itself already succeeded.
                    var db = scope.ServiceProvider.GetService<DbContext>();
                    if (db is not null)
                    {
                        var metadata = scope.ServiceProvider.GetService<IMetadataServerService>();
                        var logger = scope.ServiceProvider.GetService<ILogger<WhisparrSync>>() ?? (ILogger)NullLogger.Instance;
                        await SceneEnricher.EnrichAsync(db, metadata, scan, logger, coveId, id, path, ct);
                    }
                }
            ...
            {
                // The sync-broken signal: the path passed the root guard (a legitimate Whisparr path) yet Cove
                // can't open it — almost always a path mismatch. A distinct reason string so the settings
                // surface can warn; the scoped-scan fallback still runs.
                return Fallback(scan, path, "Video", PathNotVisibleReason);
            }
            catch (Exception ex) when (ex is NotSupportedException or IOException)
            {
                // Another IO/format failure (not a missing path) — classify-not-throw and fall back to a scan.
