...
public sealed partial class WhisparrSync
{
        ...
        MonitorRequest req, WhisparrClient client, ICurrentPrincipalAccessor principal, CancellationToken ct)
    {
        ...
        if (ResolveRemoteId(req.RemoteIds, options) is not { } stashId)
        {
            ...
            return Results.Json(new { code = "NO_STASHDB_IDENTITY" }, MonitorResponseJsonOptions);
        }
        ...
        SceneStatusBatchRequest req, WhisparrClient client, ICurrentPrincipalAccessor principal, CancellationToken ct)
    {
        ...
        if (coveIds.Length == 0)
        {
            return Results.Json(new { states = new Dictionary<int, SceneWhisparrState>() }, SceneStatusBatchJsonOptions);
        }
    ...
    /// so it is unit-testable host-free and re-uses the cached lists). Classifies each scene with the SAME
    /// <see cref="SceneStatusProjector.Classify"/> the summary + scene panel use, so the card badge, toolbar count,
    /// and scene tab never disagree. Empty inputs (a failed upstream read) classify every scene as <c>notAdded</c>.
    /// </summary>
    internal static async Task<IReadOnlyDictionary<int, SceneWhisparrState>> SceneStatusBatchCoreAsync(
        ...
        ICoveLibraryPort library, CancellationToken ct)
    {
        ...
        var states = new Dictionary<int, SceneWhisparrState>();
        foreach (var video in videos)
        {
            states[video.CoveId] = SceneStatusProjector.Classify(video.StashIds, index, excluded);
        }
        ...
        ReflectOwnedRequest req, WhisparrClient client, ICurrentPrincipalAccessor principal, CancellationToken ct)
    {
        ...
        {
            // v3 defers owned-import this release, and an unmanageable version has no adapter — both refuse
            // BEFORE resolving the DB scope or reading any Cove scene (wire-free).
            return Results.Json(new { code = "VERSION_UNSUPPORTED" }, statusCode: 400);
        }
        ...
        SceneGrabReleaseRequest req, WhisparrClient client, ICurrentPrincipalAccessor principal, CancellationToken ct)
    {
        ...
        var result = await new SceneActions(client, options, EmptyCoveLibraryPort.Instance)
            .GrabReleaseAsync(req.Guid, req.IndexerId, ct);
        if (result.IsOk)
        {
