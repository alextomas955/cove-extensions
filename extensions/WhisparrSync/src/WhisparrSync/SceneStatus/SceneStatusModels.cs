...
/// type-level converter carries the casing on every options object, including the plain-Web one that registers
/// no enum converter. The options objects that do register one use the same camelCase policy; the two agree.
/// <c>WireEnumCasingTests</c> asserts each member's literal wire value through both.
/// </remarks>
