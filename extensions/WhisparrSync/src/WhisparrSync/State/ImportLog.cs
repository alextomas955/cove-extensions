...
/// <summary>
/// One audited auto-import attempt (IMPT-04). <see cref="UtcTicks"/> is a server-written
...
/// <summary>
/// The IMPT-04 audit journal: append-only entries persisted as a SINGLE JSON-array blob over one
...
internal sealed class ImportLog(IExtensionStore store)
{
    ...
    private static ImportLogEntry[] Parse(string? json)
    {
        ...
        {
            return []; // corrupt/hand-edited blob → empty journal, never throws (IMPT-04 resilience)
        }
    }
}
