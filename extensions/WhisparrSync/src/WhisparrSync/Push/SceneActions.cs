...
internal sealed class SceneActions(WhisparrClient client, WhisparrOptions options, ICoveLibraryPort library)
{
    ...
    /// </summary>
    internal Task<WhisparrResult<bool>> GrabReleaseAsync(string guid, int indexerId, CancellationToken ct)
    {
        var adapter = SelectAdapter();
        return adapter is null
            ? Task.FromResult(WhisparrResult<bool>.VersionMismatch(options.DetectedVersion))
            : adapter.GrabReleaseAsync(options.BaseUrl, options.ApiKey, guid, indexerId, ct);
    }
    ...
    /// by that id, attaches the owned file via a targeted <c>ManualImport</c> — Cove's own file is never moved or
    /// deleted (v2 registers in place; v3 copies into the movie folder) and it never searches. The Whisparr scene
    /// set comes from <see cref="IWhisparrAdapter.ListMoviesAsync"/>
        ...
        EntityKind kind, int coveEntityId, CancellationToken ct)
    {
        ...
        foreach (var video in videos)
        {
            ...
            var import = await adapter.ImportOwnedSceneAsync(
                options.BaseUrl, options.ApiKey, scene, filePath, ct);
            if (import.IsOk)
            {
                succeeded++;
            }
        ...
        return total == 0
            ? WhisparrResult<BulkActionResult>.Ok(BulkActionResult.Empty)
            : WhisparrResult<BulkActionResult>.Ok(new BulkActionResult(total, succeeded, failed));
    }
    ...
    // Resolve the two add prerequisites in one shot: the root path + the origin tag id. Either
    // resolve failing propagates verbatim so a bad key / unreachable / missing-root surfaces to the caller.
    private async Task<WhisparrResult<AddContext>> ResolveAddContextAsync(CancellationToken ct)
    {
        var rootResult = await _addContext.ResolveRootFolderPathAsync(ct);
        if (!rootResult.IsOk)
        {
