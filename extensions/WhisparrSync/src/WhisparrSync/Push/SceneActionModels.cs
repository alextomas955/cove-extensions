...
/// </summary>
/// <remarks>
/// An item that is neither succeeded nor failed was SKIPPED — today, an add the user's own Whisparr
/// import exclusions cover. The skip count is derived from the three counters, keeping the wire shape at
/// three; its human-facing form travels in <see cref="Message"/>.
/// </remarks>
internal sealed record BulkActionResult(int Total, int Succeeded, int Failed, string? Message = null)
{
    ...
    internal int Skipped => Total - Succeeded - Failed;

    /// <summary>Every attempted item was skipped — work was requested, none of it succeeded OR failed.</summary>
    internal bool AllSkipped => Total >= 1 && Succeeded == 0 && Failed == 0;
}
