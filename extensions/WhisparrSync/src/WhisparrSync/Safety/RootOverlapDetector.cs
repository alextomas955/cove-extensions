...
internal static class RootOverlapDetector
{
    ...
    // separator boundary (so "/data/media-evil" does NOT count as beneath "/data/media"). Both paths MUST already
    // be normalized through EventLedger.NormalizePath. Shared with AddContextResolver's root derivation so the
    // segment-boundary containment rule is single-sourced, never a fresh raw compare.
