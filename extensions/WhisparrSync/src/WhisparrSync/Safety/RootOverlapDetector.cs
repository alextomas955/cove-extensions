...
/// <summary>
/// Re-grab-loop guard: detects when a Cove library root overlaps a Whisparr root — either direction
...
/// trimmed, case-SENSITIVE — the Linux/Docker target), then containment is tested at a SEGMENT BOUNDARY so a sibling like
...
internal static class RootOverlapDetector
{
    ...
    /// </summary>
    /// <remarks>
    /// The boundary is the point of the rule: <c>/data/media-evil</c> must NOT count as beneath
    /// <c>/data/media</c>, or a file on a sibling disk would be routed to the wrong Whisparr root. Both paths MUST
    /// already be normalized through <see cref="EventLedger.NormalizePath"/>. <c>AddContextResolver</c>'s root
    /// derivation and its path translation share this one rule; neither re-derives it.
    /// </remarks>
