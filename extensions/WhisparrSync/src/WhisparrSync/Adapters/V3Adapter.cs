...
/// wire knowledge (which client endpoint answers each port method) lives here, not in the handlers, so
/// <see cref="V2Adapter"/> sits behind the same <see cref="IWhisparrAdapter"/> port without touching callers.
/// </summary>
internal class V3Adapter(WhisparrClient client, TimeSpan? monitorSettleDelay = null)
    : WhisparrAdapterBase(client),
        IWhisparrAdapter,
        IWhisparrPerformerMonitor,
        IWhisparrScenePush,
        IWhisparrSceneLookup,
        IWhisparrExclusions,
        IWhisparrReleaseGrab,
        IWhisparrFileSettings,
        IWhisparrStatusIndexSource
{
    ...
    public async Task<WhisparrResult<IReadOnlyDictionary<string, WhisparrMovieFacts>>> LoadStatusIndexAsync(
        string baseUrl, string apiKey, CancellationToken ct)
    {
        // The keying comes from the projector's own single-row seam, so a folded index and a materialized one
        // cannot come to key the same row differently.
        var folded = await Client.FoldMovieFactsAsync(
            baseUrl,
            apiKey,
    ...
    public async Task<WhisparrResult<WhisparrMovie?>> FindSceneMovieAsync(
        string baseUrl, string apiKey, IReadOnlyList<string> stashIds, CancellationToken ct)
    {
        foreach (var stashId in stashIds)
        {
            ...
            var result = await Client.GetMovieByStashIdAsync(baseUrl, apiKey, stashId, ct);
            if (!result.IsOk)
            {
                return Propagate<WhisparrMovie[], WhisparrMovie?>(result);
            }

            if (Array.Find(result.Value!, row => SceneStatusProjector.IsMovieFor(row, [stashId])) is { } movie)
            {
                return WhisparrResult<WhisparrMovie?>.Ok(movie);
            }
        }

        return WhisparrResult<WhisparrMovie?>.Ok(null);
    }
    ...
    public Task<WhisparrResult<EntityMonitorResult>> SetStudioMonitorAsync(
        ...
        => SetEntityMonitorAsync(
            baseUrl, apiKey, EntityKind.Studio, stashId, monitored, scope, rootFolderPath, qualityProfileId, tagIds, ct);

    public Task<WhisparrResult<EntityMonitorResult>> SetPerformerMonitorAsync(
        string baseUrl, string apiKey, string stashId, bool monitored, MonitorScope scope,
        string rootFolderPath, int qualityProfileId, IReadOnlyList<int> tagIds, CancellationToken ct)
        => SetEntityMonitorAsync(
            baseUrl, apiKey, EntityKind.Performer, stashId, monitored, scope, rootFolderPath, qualityProfileId, tagIds, ct);

    // The kind-parameterized monitor spine both role methods above share: flip the container, then (only on ON)
    // fire the Cove-owned population refresh and reconcile the attributed scenes to the chosen scope. The
    // studio/performer split is an interface-presence concern; internally one body keeps the two flows symmetric.
    private async Task<WhisparrResult<EntityMonitorResult>> SetEntityMonitorAsync(
        ...
        string rootFolderPath, int qualityProfileId, IReadOnlyList<int> tagIds, CancellationToken ct)
    {
        var flip = kind == EntityKind.Studio
            ? await FlipStudioMonitorAsync(baseUrl, apiKey, stashId, monitored, rootFolderPath, qualityProfileId, tagIds, ct)
            : await FlipPerformerMonitorAsync(baseUrl, apiKey, stashId, monitored, rootFolderPath, qualityProfileId, tagIds, ct);
        if (!flip.IsOk)
        {
    ...
    public Task<WhisparrResult<EntityMonitorResult>> RegisterStudioAsync(
    ...
    // status uses (studio and performer alike by foreign id), optionally to monitored. No StashDB call.
    public Task<WhisparrResult<int[]>> ListStudioAttributedIdsAsync(
        string baseUrl, string apiKey, string stashId, bool monitoredOnly, CancellationToken ct)
        => CollectAttributedEntityIdsAsync(baseUrl, apiKey, EntityKind.Studio, stashId, monitoredOnly, filelessOnly: false, ct);

    public Task<WhisparrResult<int[]>> ListPerformerAttributedIdsAsync(
        string baseUrl, string apiKey, string stashId, bool monitoredOnly, CancellationToken ct)
        => CollectAttributedEntityIdsAsync(baseUrl, apiKey, EntityKind.Performer, stashId, monitoredOnly, filelessOnly: false, ct);
    ...
    // This reads the FULL movie set (GET /api/v3/movie) and filters in-memory, and the scoped alternative does
    // exist: the two listBy*ForeignId routes name an entity's movie ids and the by-id hydration answers full
    // rows, HasFile included. It stays whole-set because its consumers are the grab-capable search surfaces,
    // which is a separate change with its own loop-safety surface to re-verify. The condition that moves it is
    // those surfaces being repointed, not a missing endpoint.
    ...
    private async Task<WhisparrResult<EntityFlip>> FlipStudioMonitorAsync(
    ...
    private async Task<WhisparrResult<EntityFlip>> FlipPerformerMonitorAsync(
        string baseUrl, string apiKey, string stashId, bool monitored,
        string rootFolderPath, int qualityProfileId, IReadOnlyList<int> tagIds, CancellationToken ct)
    {
    ...
    public async Task<WhisparrResult<EntityStatus>> GetStudioStatusAsync(
        string baseUrl, string apiKey, string stashId, CancellationToken ct)
    {
        var getResult = await Client.GetStudioByStashIdAsync(baseUrl, apiKey, stashId, ct);
        if (!getResult.IsOk)
    ...
    public async Task<WhisparrResult<EntityStatus>> GetPerformerStatusAsync(
        string baseUrl, string apiKey, string stashId, CancellationToken ct)
    {
        var getResult = await Client.GetPerformerByStashIdAsync(baseUrl, apiKey, stashId, ct);
    ...
    // The single studio attribution predicate: a movie belongs to the studio when its studioForeignId is the
    // studio's own StashDB id — the same identity rule the performer predicate below uses, so a studio whose
    // title in Whisparr differs from Cove's still attributes its scenes, and a studio with no title at all is
    // still attributable. Shared by the catalogue enumeration and the attributed-id collector so the rows a
    // surface counts and the ids a search acts on can never diverge.
    private protected static Func<WhisparrMovie, bool> StudioAttribution(WhisparrStudio studio)
        => movie => studio.ForeignId is { Length: > 0 } foreignId
            && string.Equals(movie.StudioForeignId, foreignId, StringComparison.OrdinalIgnoreCase);
    ...
    private protected static Func<WhisparrMovie, bool> PerformerAttribution(string stashId)
    ...
    // failed read/write propagates verbatim (state + diagnostic) rather than inventing a success. Only the two
    // states this adapter handles inline on the paths that declare them are re-mapped: an Absent/Conflict
    // reaching here means a path did NOT handle it, so it collapses to a classified Unreachable rather than a
    // silent Ok. Everything else defers to the shared state→failure mapping, so a state added there (Excluded)
    // cannot be silently dropped by a second switch here.
    private protected static WhisparrResult<TTo> Propagate<TFrom, TTo>(WhisparrResult<TFrom> source)
        => source.State switch
        {
            ...
            _ => WhisparrResult<TTo>.PropagateFrom(source),
        };
}
