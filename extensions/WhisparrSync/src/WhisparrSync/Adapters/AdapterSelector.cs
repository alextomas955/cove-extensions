...
/// <summary>
/// The fail-closed major-version gate. Both Whisparr v2 and v3 answer
...
internal static class AdapterSelector
{
    ...
    /// unrecognizable version fails closed at the gate rather than defaulting to any adapter.
    /// </summary>
    ...
    /// A <c>null</c> return is the refusal — the caller surfaces a typed version-mismatch, never a
    /// wrong-adapter call.
    /// </summary>
    ...
    /// <summary>
    /// Selects the adapter for an already-persisted selection AND a resolved capability answer: a
    /// <see cref="V3EntityCatalogueAdapter"/> for <c>"v3"</c> when the instance declares the narrow catalogue
    /// routes, a plain <see cref="V3Adapter"/> for <c>"v3"</c> when it does not, a <see cref="V2Adapter"/> for
    /// <c>"v2"</c>, otherwise <c>null</c> (refuse).
    /// </summary>
    internal static IWhisparrAdapter? SelectForVersion(
        string? selectedVersion, WhisparrClient client, WhisparrCapabilities capabilities,
        TimeSpan? monitorSettleDelay = null)
        => string.Equals(selectedVersion, "v3", StringComparison.OrdinalIgnoreCase) && capabilities.NarrowEntityCatalogue
            ? new V3EntityCatalogueAdapter(client, monitorSettleDelay)
            : SelectForVersion(selectedVersion, client, monitorSettleDelay);
    ...
    /// <see cref="V2Adapter"/> for <c>"v2"</c>, otherwise <c>null</c> (refuse). Case-insensitive.
    /// </summary>
    /// <remarks>
    /// This overload resolves no capability, so its v3 answer is the plain <see cref="V3Adapter"/> — a call
    /// site that never asked the instance what it can do structurally cannot reach a capability-gated role.
    /// That is the property, not an omission: reaching the role requires taking the overload above.
    /// </remarks>
    internal static IWhisparrAdapter? SelectForVersion(
        string? selectedVersion, WhisparrClient client, TimeSpan? monitorSettleDelay = null)
    {
        if (string.Equals(selectedVersion, "v3", StringComparison.OrdinalIgnoreCase))
        {
            return new V3Adapter(client, monitorSettleDelay);
        }

        if (string.Equals(selectedVersion, "v2", StringComparison.OrdinalIgnoreCase))
        {
            return new V2Adapter(client, monitorSettleDelay);
        }
