...
/// origin tag (attribution), resolves the root-folder path from the stored
/// <see cref="WhisparrOptions.RootFolderId"/>, selects the version adapter, and delegates the add-then-flip /
/// status projection to it. Keeping the tag-ensure + root-resolution + adapter-selection here means the
/// endpoints call ONE method and hold no wire semantics. Constructor-injected with the
/// transport client + the already-loaded options, so it unit-tests against a fake HTTP handler with no host.
/// </summary>
internal sealed class EntityMonitor(WhisparrClient client, WhisparrOptions options, TimeSpan? monitorSettleDelay = null)
{
        ...
        EntityKind kind, string stashId, bool monitored, MonitorScope scope, CancellationToken ct)
    {
        ...
        if (monitored)
        {
            // Resolve the stored RootFolderId to its path via the live root-folder list — never a
            // hand-typed path. Only needed for the add leg, so this work is skipped entirely on OFF.
