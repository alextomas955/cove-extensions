...
/// <summary>
/// The auto-register header-token wiring (closes the Phase-1 403 finding): the v3 Notification payload
/// carries the shared secret in a <c>headers</c> entry keyed <c>X-Cove-Token</c>, which is exactly the
/// header the 03-01 receiver validates. So Whisparr's Test ping — which posts to the URL with the
