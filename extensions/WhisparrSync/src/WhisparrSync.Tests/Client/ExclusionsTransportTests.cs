...
public sealed class ExclusionsTransportTests
{
    ...
    // Structural defer: v2 never holds IWhisparrExclusions or IWhisparrReleaseGrab, so the exclusion and
    // release-grab verbs have no method to call on a v2 instance — the deferral is a type-system fact.
    [Fact]
    public void V2Adapter_ImplementsNeitherExclusionsNorReleaseGrab()
    {
        var v2 = typeof(V2Adapter).GetInterfaces();
        Assert.DoesNotContain(typeof(IWhisparrExclusions), v2);
        Assert.DoesNotContain(typeof(IWhisparrReleaseGrab), v2);
    }
}
