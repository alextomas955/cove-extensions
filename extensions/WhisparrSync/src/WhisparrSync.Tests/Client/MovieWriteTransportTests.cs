...
public sealed class MovieWriteTransportTests
{
    ...
    [Fact]
    public async Task Create_movie_excluded_studio_500_classifies_as_excluded_carrying_whisparr_message()
    {
        // Measured against Whisparr 3.3.4: an add whose studio is import-excluded answers HTTP 500 with
        // this exact envelope from AddMovieService.SetPropertiesAndValidate. It must classify Excluded (a
        // user-config skip), never Rejected/Unreachable, or the bulk loops count the user's own choice as a
        // failure.
        var errorBody = JsonSerializer.Serialize(new
        {
            message = "Studio: [Hussie Pass] has been excluded",
            description = "NzbDrone.Core.Exceptions.ExcludedException: Studio: [Hussie Pass] has been excluded\n   at NzbDrone.Core.Movies.AddMovieService.SetPropertiesAndValidate(Movie newMovie)",
        });
        var handler = new FakeHttpMessageHandler(HttpStatusCode.InternalServerError, "application/json", errorBody);
    ...
    [Fact]
    public async Task Create_movie_excluded_classifies_from_message_alone_when_description_is_absent()
    {
        // The description (which names ExcludedException) is the stronger marker but is not guaranteed on
        // every build, so the message clause alone must still classify Excluded.
        var errorBody = JsonSerializer.Serialize(new { message = "Studio: [Shower 4K] has been excluded" });
        var handler = new FakeHttpMessageHandler(HttpStatusCode.InternalServerError, "application/json", errorBody);

        var result = await ClientFor(handler).CreateMovieAsync(BaseUrl, ApiKey, "{\"foreignId\":\"x\"}", CancellationToken.None);

        Assert.Equal(WhisparrResultState.Excluded, result.State);
        Assert.Equal("Studio: [Shower 4K] has been excluded", result.Reason);
    }

    [Fact]
    public async Task Create_movie_unrelated_500_stays_rejected_not_excluded()
    {
        // The exclusion signal is the BODY, never the 500 status — a genuine server fault must keep
        // surfacing as a failure rather than being silently counted as a user-configured skip.
        var errorBody = JsonSerializer.Serialize(new { message = "Object reference not set to an instance of an object." });
        var handler = new FakeHttpMessageHandler(HttpStatusCode.InternalServerError, "application/json", errorBody);
