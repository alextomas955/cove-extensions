...
public sealed class MovieWriteTransportTests
{
    ...
    [Fact]
    public async Task Create_movie_400_genuine_bad_body_stays_unreachable_not_conflict()
    {
        // A real bad-request (NOT the already-added case) must NOT be swallowed as Conflict — only the
        // MovieExists body is the idempotency signal; every other 400 remains a classified Unreachable.
        var errorBody = JsonSerializer.Serialize(new[]
        {
