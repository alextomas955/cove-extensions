...
public sealed class SceneAdapterTests
{
    ...
    // ---- (6) attribution: studio and performer alike by ForeignId + monitoredOnly, no StashDB ----

    [Fact]
    public async Task ListAttributedMovieIds_studio_attributes_by_foreign_id_and_filters_monitored()
    {
        var movies = JsonSerializer.Serialize(new[]
        {
            new { id = 1, studioForeignId = StudioStashId, studioTitle = "IEnergy", monitored = true },
            // The title Whisparr holds for this row differs from the studio's own; the identity is what ties it.
            new { id = 2, studioForeignId = StudioStashId, studioTitle = "IEnergy Productions", monitored = false },
            new { id = 3, studioForeignId = "another-studio-id", studioTitle = "Other Studio", monitored = true },
        });

        // monitoredOnly = false -> both rows carrying the studio identity (1, 2), never the other studio's row.
        var all = await V3(FakeHttpMessageHandler.Sequence(Ok($"[{StudioRow(7, "IEnergy")}]"), Ok(movies)))
            .ListStudioAttributedIdsAsync(
BaseUrl, ApiKey, StudioStashId, monitoredOnly: false, CancellationToken.None);
        ...
        // monitoredOnly = true -> only the monitored row carrying the studio identity (1).
        ...
        var monitored = await V3(monitoredHandler)
            .ListStudioAttributedIdsAsync(
BaseUrl, ApiKey, StudioStashId, monitoredOnly: true, CancellationToken.None);
    ...
    public async Task ListAttributedMovieIds_performer_attributes_by_foreign_id()
    {
        ...
        var result = await V3(handler)
            .ListPerformerAttributedIdsAsync(BaseUrl, ApiKey, PerformerStashId, monitoredOnly: false, CancellationToken.None);
...
BaseUrl, ApiKey, StudioStashId, monitoredOnly: false, CancellationToken.None);
    ...
    // Structural defer: v2 has no POST /episode, so it never holds IWhisparrScenePush — both the per-scene
    // add and the per-scene monitor have no method to call on a v2 instance (wire-free by construction).
    [Fact]
    ...
    public async Task V2_list_attributed_movie_ids_studio_enumerates_the_sites_episode_ids()
    {
        ...
        var result = await V2(handler).ListStudioAttributedIdsAsync(
BaseUrl, ApiKey, "3372", monitoredOnly: false, CancellationToken.None);

        Assert.Equal(WhisparrResultState.Ok, result.State);
        Assert.Equal([1, 2, 3], result.Value!);
    }

    // Structural defer: a performer has no v2 entity, so v2 never holds IWhisparrPerformerMonitor and the
    // performer attributed-id enumeration has no method to call on a v2 instance.
    [Fact]
