...
/// <summary>
/// The MATCH-04 persistence contract for <see cref="MatchStateStore"/> over a fake IExtensionStore:
...
public sealed class MatchStateStoreTests
{
    ...
    public async Task RoundTrip_PreservesEntries()
    {
        ...
        var entries = new[]
        {
            ...
            Entry(11, 2, "", MatchStatus.NeedsReview, MatchedBy.Fuzzy),
        };
        ...
        Assert.Contains(loaded, e => e.CoveId == 11 && e.MatchedBy == MatchedBy.Fuzzy
    ...
    public async Task Blob_IsSingleKey_WithStringRenderedEnums()
    {
        var store = new FakeStore();
        var subject = new MatchStateStore(store);
        await subject.SetAllAsync([Entry(10, 1, "uuid-a", MatchStatus.NeedsReview, MatchedBy.Fuzzy)]);

        var all = await store.GetAllAsync();
        Assert.Equal([StoreKey], all.Keys);

        var blob = all[StoreKey];
        Assert.Contains("\"Fuzzy\"", blob);
