...
public sealed class SceneStatusProjectorTests
{
    ...
    // --- The single-row predicate agrees with the index it is extracted from ---

    [Theory]
    // A scene-typed row keyed by its foreignId.
    [InlineData("scene", null, "wanted-id", true)]
    // Any type keyed by its own StashDB id.
    [InlineData("movie", "wanted-id", "other-foreign", true)]
    [InlineData("scene", "wanted-id", "other-foreign", true)]
    // A movie-typed row whose foreignId is a tmdbId that happens to read like the asked id: the index does
    // NOT key it, so neither may the predicate.
    [InlineData("movie", "other-stash", "wanted-id", false)]
    // A row matching on neither field.
    [InlineData("scene", "other-stash", "other-foreign", false)]
    // Casing is not a difference on either side.
    [InlineData("scene", null, "WANTED-ID", true)]
    public void IsMovieFor_answers_exactly_what_the_index_plus_FindMovie_answers(
        string itemType, string? stashId, string? foreignId, bool expected)
    {
        var movie = Movie(1, stashId, monitored: true, hasFile: false, itemType: itemType, foreignId: foreignId);
        string[] stashIds = ["wanted-id"];

        var viaIndex = SceneStatusProjector.FindMovie(stashIds, SceneStatusProjector.BuildMovieIndex([movie]));

        Assert.Equal(expected, SceneStatusProjector.IsMovieFor(movie, stashIds));
        Assert.Equal(expected, viaIndex is not null);
    }

    [Fact]
    public void IsMovieFor_ignores_empty_ids_on_both_sides()
    {
        var idless = Movie(1, stashId: null, monitored: true, hasFile: false, itemType: "scene", foreignId: null);
        var real = Movie(2, "real-id", monitored: true, hasFile: false);

        Assert.False(SceneStatusProjector.IsMovieFor(idless, ["real-id"]));
        Assert.False(SceneStatusProjector.IsMovieFor(real, [""]));
        Assert.False(SceneStatusProjector.IsMovieFor(real, []));
    }
