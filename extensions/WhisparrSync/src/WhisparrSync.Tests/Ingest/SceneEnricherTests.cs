...
public sealed class SceneEnricherTests
{
    ...
    private static async Task<int> SeedBareVideoAsync(DbContext db)
    {
    ...
    private static Video Reload(DbContext db, int coveId)
    ...
    public async Task Enrich_StampsRemoteId_Identifies_AndTriggersScanGenerate()
    {
        ...
        try
        {
            ...
            await SceneEnricher.EnrichAsync(db, meta, scan, NullLogger.Instance, coveId, Identity, "/media/scene.mp4", default);
    ...
    public async Task Enrich_AlreadyIdentified_IsNoOp_IdentifyAndScanRunExactlyOnce()
    {
        ...
        try
        {
            ...
            await SceneEnricher.EnrichAsync(db, meta, scan, NullLogger.Instance, coveId, Identity, "/media/s.mp4", default);
            await SceneEnricher.EnrichAsync(db, meta, scan, NullLogger.Instance, coveId, Identity, "/media/s.mp4", default);
    ...
    [Fact]
    public async Task Enrich_MetadataServerThrows_EmitsExactlyOneDiagnosticLine()
    {
        var (db, conn) = await CoveContextFactory.CreateSqliteContextAsync();
        try
        {
            ...
            await SceneEnricher.EnrichAsync(db, meta, scan, logger, coveId, Identity, "/media/s.mp4", default);

            var entry = Assert.Single(logger.Entries);
            Assert.Equal(LogLevel.Warning, entry.Level);
            Assert.Contains($"coveId={coveId}", entry.Message, StringComparison.Ordinal);
            Assert.Contains($"endpoint={StashEndpoint}", entry.Message, StringComparison.Ordinal);
