...
public sealed class ReconStatusTests
{
    ...
    private static string Movies() => JsonSerializer.Serialize(new[]
    {
        ...
        new { id = 20, title = "Downloaded Scene", year = 2021, stashId = "uuid-dl", foreignId = "uuid-dl", itemType = "scene", monitored = true, hasFile = true },
    ...
    {
        // Exclusion-first: movie 10 has a file (would be Downloaded) but its StashId is in the exclusion read.
        var response = await RunAsync(Movies(), Exclusions());
        Assert.Equal(SceneWhisparrState.Excluded, StateOf(response, 10));
    }

    [Fact]
    public async Task NonExcludedMovieWithFile_Row_IsDownloaded()
    {
        var response = await RunAsync(Movies(), Exclusions());
        Assert.Equal(SceneWhisparrState.Downloaded, StateOf(response, 20));
    }
    ...
    [Fact]
    public async Task EmptyExclusionRead_LeavesTheFileMovieDownloaded()
    {
        // A v2 instance defers the exclusion read (empty set here stands in for that degrade): no "excluded"
        // rows, the movie is classified purely on its own facts.
        var response = await RunAsync(Movies(), "[]");
        Assert.Equal(SceneWhisparrState.Downloaded, StateOf(response, 10));
    }
    ...
    public async Task ReconRow_WhisparrState_SerializesAsCamelCase()
    {
        ...
        Assert.Contains("\"whisparrState\":\"downloaded\"", json, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain("\"whisparrState\":\"Downloaded\"", json, StringComparison.Ordinal);
    }
