...
/// <summary>
/// The library-sync loop-safety contract: the "Sync my library to Whisparr" fan-out can NEVER
...
public sealed class SyncLibraryLoopSafetyTests
{
    ...
    public void SyncOp_HasNoGrabMember()
    {
        ...
        Assert.Equal(
            new[]
            {
                ...
                nameof(Ext.SyncOp.AddSlice), nameof(Ext.SyncOp.RegisterEntity),
            },
    ...
    public void BuildSyncUnits_NeverPlansAGrabOp()
    {
        ...
        var slices = Ext.PlanSceneSlices(1, []);

        var units = Ext.BuildSyncUnits(
            studios, performers, slices, alsoMonitor: true, MonitorScope.AllScenes,
            new WhisparrOptions { SelectedVersion = "v3" }, Adapter("v3"));

        Assert.NotEmpty(units);
        Assert.All(units, u => Assert.True(
            u.Op is Ext.SyncOp.ReflectOwned or Ext.SyncOp.Monitor or Ext.SyncOp.AddSlice or Ext.SyncOp.RegisterEntity));
    }
