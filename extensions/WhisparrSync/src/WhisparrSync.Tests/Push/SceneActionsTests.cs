...
public sealed class SceneActionsTests
{
    ...
    // video with that StashDB id + a file attaches to it via a targeted ManualImport at movieId — in place.
    private const string OwnedV3FilePath = "/data/media/A Scene/scene.mkv";
    ...
    [Fact]
    public async Task ReflectOwned_v3_imports_the_owned_file_to_its_fileless_movie_in_place()
    {
        var library = new FakeCoveLibraryPort();
        ...
        var handler = FakeHttpMessageHandler.Sequence(
            ...
            Ok(ManualImportListing(OwnedV3FilePath)),                                // import: GET /manualimport
            Ok("{}"),                                                                //         POST /command (ManualImport)
        ...
        // A targeted ManualImport at the matched movie — NEVER a search (loop-safe). importMode "copy" (never
        // "move"/"auto") is the safety invariant: Cove's own file is never relocated by the import.
        var command = Assert.Single(handler.Requests, r => r.Url.EndsWith("/api/v3/command", StringComparison.Ordinal));
        Assert.Contains("\"name\":\"ManualImport\"", command.Body);
        Assert.Contains("\"movieId\":42", command.Body);
        Assert.Contains("\"importMode\":\"copy\"", command.Body);
        Assert.DoesNotContain("\"importMode\":\"move\"", command.Body!);
        Assert.DoesNotContain("Search", command.Body!);
        AssertNoStashDbCall(handler);
    }
