...
public sealed class ReflectOwnedTests
{
    ...
    // The per-entity read resolves the entity's OWN remote id server-side, so every fixture here seeds one.
    private static SceneActions ActionsFor(FakeHttpMessageHandler handler, FakeCoveLibraryPort library)
    {
        ...
        return SceneActionsFactory.Build(new WhisparrClient(new HttpClient(handler)), V3Options, library);
    }
    ...
    public async Task FolderPerScene_adopts_each_scene_in_place_no_flat_message()
    {
        ...
        // The entity's own catalogue, served out of the sequence above: the two fileless rows the whole-set read
        // used to return for this studio.
        handler.ServingCatalogue(MovieList(Movie(42, StashA, hasFile: false), Movie(43, StashB, hasFile: false)));
