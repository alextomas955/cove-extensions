...
public sealed class SceneExclusionActionsTests
{
    ...
    // ---- GrabRelease POSTs /api/v3/release with the guid+indexerId body ----

    [Fact]
    public async Task GrabRelease_posts_release_with_guid_and_indexer_id()
    {
        var handler = FakeHttpMessageHandler.Sequence(Status(HttpStatusCode.OK));

        var result = await ActionsFor(handler).GrabReleaseAsync("indexer-guid-1", 3, CancellationToken.None);
    ...
    public async Task V2_options_defer_every_new_verb_without_a_wire_call()
    {
        ...
        var grab = await ActionsFor(grabHandler, v2).GrabReleaseAsync("g", 1, CancellationToken.None);
