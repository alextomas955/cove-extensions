...
/// <summary>
/// The connector-sourced webhook find + idempotent register wire contract. Drives the
...
/// unique-name-400/409-as-success rule, the connector read, and the token-safety invariant: the
...
public sealed class WebhookConnectionTests
{
    ...
    public async Task Register_V3_ExistingConnection_GetThenPut_ReMintsTokenNoSecondPost()
    {
        ...
        // Token safety: the PUT body re-mints from the PASSED url, never the stale stored one, and
