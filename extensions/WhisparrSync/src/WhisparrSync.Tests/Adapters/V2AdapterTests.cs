...
public sealed class V2AdapterTests
{
    ...
    public async Task SetEntityMonitor_Studio_AbsentOn_AddsNonGrabbing_ThenFlips_NoSearchCommand()
    {
        ...
        var result = await AdapterFor(handler).SetStudioMonitorAsync(
    ...
    public async Task GetEntityStatus_Studio_CountsGrabbedOfTotalFromEpisodes()
    {
        ...
        var result = await AdapterFor(handler).GetStudioStatusAsync(
    ...
    public async Task GetEntityStatus_Studio_AbsentSite_IsAddedFalse_ZeroOfZero()
    {
        ...
        var result = await AdapterFor(handler).GetStudioStatusAsync(
            BaseUrl, ApiKey, AbsentTpdb, CancellationToken.None);
    ...
    // Structural defer: v2 has NO performer entity (performers are embedded episode.actors metadata), so a
    // v2 adapter never holds IWhisparrPerformerMonitor — the performer monitor + status + attributed-id verbs have
    // no method to call, which is the wire-free deferral encoded in the type system.
    [Fact]
    ...
    public async Task ListAttributedMovieIds_Studio_MonitoredOnly_ReturnsMonitoredEpisodeIds()
    {
        ...
        var result = await AdapterFor(handler).ListStudioAttributedIdsAsync(
    ...
    public async Task ListAttributedMovieIds_Studio_All_ReturnsEveryEpisodeId()
    {
        ...
        var result = await AdapterFor(handler).ListStudioAttributedIdsAsync(
            BaseUrl, ApiKey, AddedTpdb, monitoredOnly: false, CancellationToken.None);
    ...
    public async Task ListAttributedMovieIds_Studio_AbsentSite_ReturnsEmpty()
    {
        ...
        var result = await AdapterFor(handler).ListStudioAttributedIdsAsync(
            BaseUrl, ApiKey, AbsentTpdb, monitoredOnly: false, CancellationToken.None);
    ...
    // --- Scene-level + release-grab members DEFER structurally: v2 holds neither role interface ---

    // v2 has no per-scene (episode) add — no POST /episode — so it never holds IWhisparrScenePush; the per-scene
    // add and monitor verbs have no method to call.
    [Fact]
    ...
    // endpoint, so it never holds IWhisparrReleaseGrab.
    [Fact]
