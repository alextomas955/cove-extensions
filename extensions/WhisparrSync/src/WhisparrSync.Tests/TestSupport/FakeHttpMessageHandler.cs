...
internal sealed class FakeHttpMessageHandler : HttpMessageHandler
{
    ...
    private Func<CapturedRequest, HttpResponseMessage?>? _byRequest;

    private FakeHttpMessageHandler(IReadOnlyList<Func<HttpResponseMessage>> steps) => _steps = steps;

    /// <summary>
    /// Overlays a request-keyed response ahead of the step list: a request the overlay answers is served from
    /// it and consumes no step, and everything else keeps the configured body or sequence.
    /// </summary>
    /// <remarks>
    /// A route answering differently is otherwise only expressible by turning a fixed body into an ordered
    /// sequence, which couples every unrelated assertion to call ORDER — and a fixture whose answer depends on
    /// the request BODY (a bulk read echoing the ids it was asked for) cannot be expressed that way at all. The
    /// request is still captured, so a count assertion sees the overlaid call rather than being silently
    /// relieved of it.
    /// </remarks>
    public FakeHttpMessageHandler Also(Func<CapturedRequest, HttpResponseMessage?> byRequest)
    {
    ...
    protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        var captured = new CapturedRequest(request.Method, request.RequestUri?.ToString() ?? "", LastRequestBody);
        _requests.Add(captured);
        CallCount++;

        if (_byRequest?.Invoke(captured) is { } overlaid)
        {
