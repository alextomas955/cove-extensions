...
public sealed class ReconcileTests
{
    ...
    [Fact]
    public async Task CompletedPass_PrunesHistKeysAtOrBelowTheAdvancedCheckpoint()
    {
        var (job, _, store) = NewJob(MixedHistory);
        await SeedAtZeroAsync(store);

        await job.RunAsync(default);

        // The pass ingested records 5 and 3 (recording hist:5 / hist:3) and advanced the checkpoint to 5, so
        // both hist keys are now at/below the checkpoint and provably dead — the bounded ledger drops them.
        var ledger = new EventLedger(store);
        Assert.False(await ledger.SeenAsync("hist:5"));
        Assert.False(await ledger.SeenAsync("hist:3"));

        // The cross-channel ImportKeys survive — they still guard the webhook/poll redelivery window.
        Assert.True(await ledger.SeenAsync(EventLedger.ImportKey("dl-a", "/data/media/A/A.mkv")));
        Assert.True(await ledger.SeenAsync(EventLedger.ImportKey("dl-c", "/data/media/C/C.mkv")));
    }

    [Fact]
    public async Task IncompletePass_KeepsHistKeys_SoTheReFetchWindowStaysCheap()
    {
        // The prune fires only when the checkpoint advances. A mid-paging fault leaves it un-advanced, keeping
        // the re-fetched window's hist keys alive as the cheap re-poll no-op.
        var recovered = false;
        var (job, _, store) = NewJob((page, _) => page switch
        {
            1 => Task.FromResult(OkPage(PageOf(200, ReconcileJob.PageSize))), // ids 200..151 (a full page)
            _ when !recovered => Task.FromResult(WhisparrResult<WhisparrHistoryPage>.Unreachable("boom")),
            _ => Task.FromResult(OkPage(PageOf(150, 2))),
        });
        await SeedAtZeroAsync(store);
        ...
        var afterFault = await new Checkpoint(store).LoadAsync();
        Assert.Equal(0, afterFault.LastRecordId);                        // checkpoint did not advance
        Assert.True(await new EventLedger(store).SeenAsync("hist:200")); // so page-1 hist keys were NOT pruned
    }
