...
/// <summary>
/// The scale-safety contract for the studios/performers bulk "monitor many" action. Since a v3
...
/// command queue (the StashDB-hammer the bulk design forbids). These facts drive the host-free aggregator
...
public sealed class EntityMonitorBulkPacingTests
{
    ...
    {
        new { id = 100, studioForeignId = StudioAStash, studioTitle = "StudioA", monitored = true, hasFile = false },
        new { id = 200, studioForeignId = StudioBStash, studioTitle = "StudioB", monitored = true, hasFile = false },
    });
    ...
    public async Task BulkMonitor_TwoStudios_IssuesOneSingleIdRefreshPerEntity()
    {
        ...
        var result = await Ext.EntitiesBatchCoreAsync(
            EntityKind.Studio, EntityBatchOp.Monitor, MonitorScope.NewReleases, [1, 2],
            client, library, V3Options, new WhisparrCapabilityPort(client), CancellationToken.None);
