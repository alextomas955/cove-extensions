...
public sealed class EntityMonitorTests
{
    ...
    // Two rows: a sole-offered id would satisfy a first-offered assertion by accident.
    ...
    public async Task Monitor_on_absent_studio_adds_monitored_false_then_puts_true()
    {
        var handler = FakeHttpMessageHandler.Sequence(
            ...
            Ok(QualityProfileList),                                                       // resolve the quality profile
    ...
    public async Task StudioMonitor_V3_AllScenes_BulkMonitorsAttributedScenes_NoSearch()
    {
        ...
        {
            new { id = 11, studioForeignId = StudioStashId, studioTitle = "IEnergy", monitored = false, hasFile = true },
            new { id = 12, studioForeignId = StudioStashId, studioTitle = "IEnergy", monitored = true, hasFile = false },
            new { id = 99, studioForeignId = "another-studio-id", studioTitle = "Other Studio", monitored = false, hasFile = true },
        });
        var handler = FakeHttpMessageHandler.Sequence(
            Ok(RootFolderList),
            Ok(TagListWithOrigin),
    ...
    public async Task StudioMonitor_V3_NewReleases_RefreshesThenUnmonitorsFilelessBackCatalogue()
    {
        var movies = JsonSerializer.Serialize(new[]
        {
            new { id = 11, studioForeignId = StudioStashId, studioTitle = "IEnergy", monitored = false, hasFile = true },  // owned -> untouched
            new { id = 12, studioForeignId = StudioStashId, studioTitle = "IEnergy", monitored = true, hasFile = false },  // back-catalogue -> unmonitor
            new { id = 99, studioForeignId = "another-studio-id", studioTitle = "Other Studio", monitored = false, hasFile = true },
        });
        var handler = FakeHttpMessageHandler.Sequence(
            Ok(RootFolderList),
    ...
    // Structural defer: a performer has no v2 entity, so a v2 adapter never holds IWhisparrPerformerMonitor
    // — the performer monitor + status verbs have no method to call, which is the wire-free deferral by type.
    [Fact]
