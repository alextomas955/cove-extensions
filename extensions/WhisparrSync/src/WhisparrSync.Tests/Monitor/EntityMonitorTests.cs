...
public sealed class EntityMonitorTests
{
    ...
    // The loop-safety invariant asserted on every mutating flow: no monitor-add ever targets a
    // /command path and no add/flip body sets searchForMovie — searching is deliberately out of scope for monitor-add.
    private static void AssertNoSearchTriggered(FakeHttpMessageHandler handler)
    {
        ...
        Assert.DoesNotContain(handler.Requests, r => r.Body?.Contains("searchForMovie", StringComparison.OrdinalIgnoreCase) == true);
    }
    ...
    [Fact]
    public async Task StudioMonitor_V3_NewReleases_DoesNotTouchScenes()
    {
        var handler = FakeHttpMessageHandler.Sequence(
            Ok(RootFolderList),
            Ok(TagListWithOrigin),
