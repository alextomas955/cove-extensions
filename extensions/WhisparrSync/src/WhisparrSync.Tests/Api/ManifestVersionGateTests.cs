...
public sealed class ManifestVersionGateTests
{
    ...
    [InlineData("v2")]
    [InlineData(null)]
    public void MissingTabs_PresentOnBothVersions_WithKindScopedCountEndpoint(string? version)
    {
        var manifest = new Ext().BuildManifest(version);

        var studioTab = Assert.Single(
            manifest.Tabs, t => t.PageType == "studio" && t.ComponentName == "WhisparrStudioMissingTab");
        var performerTab = Assert.Single(
            manifest.Tabs, t => t.PageType == "performer" && t.ComponentName == "WhisparrPerformerMissingTab");

        // Each tab carries its own kind-scoped count endpoint with the {entityId} placeholder the host
        // substitutes — the kind is fixed in the template, never client-derived.
        Assert.Contains("kind=studio", studioTab.CountEndpoint);
        Assert.Contains("{entityId}", studioTab.CountEndpoint);
        Assert.Contains("kind=performer", performerTab.CountEndpoint);
        Assert.Contains("{entityId}", performerTab.CountEndpoint);
    }
}
