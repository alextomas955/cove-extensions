...
public sealed class SyncLibraryJobTests
{
    ...
    private static IReadOnlyList<int> CoveIdsOf(IEnumerable<Ext.SyncUnit> units, Ext.SyncOp op, EntityKind kind)
        => [.. units.Where(u => u.Op == op && u.Op != Ext.SyncOp.AddSlice && u.Kind == kind).Select(u => u.CoveId)];

    private static IReadOnlyList<Ext.SyncSlice> SlicesOf(IEnumerable<Ext.SyncUnit> units)
        => [.. units.Where(u => u.Op == Ext.SyncOp.AddSlice).Select(u => u.Slice!.Value)];

    // What the job does between counting the library and planning: take the id sitting on each boundary
    // ordinal. Driving the production planner rather than hand-building ranges is what keeps these cases
    // pinned to the real partition.
    private static IReadOnlyList<Ext.SyncSlice> SlicesFor(IReadOnlyList<CoveVideo> videos)
    {
    ...
    [Fact]
    public async Task BuildSyncUnits_V3_WithMonitor_PlansReflectAddAndScopedMonitor_SkippingIdLess()
    {
        ...
        var units = Ext.BuildSyncUnits(
            studios, performers, SlicesFor(videos), alsoMonitor: true, MonitorScope.AllScenes, Options("v3"), Adapter("v3"));
        ...
        // One slice covers the whole library at this size, and it spans every id: eligibility is no longer a
        // planning decision, so the id-less 102 is inside the range and is skipped when the range is read.
        var slice = Assert.Single(SlicesOf(units));
    ...
    [Fact]
    public async Task BuildSyncUnits_V3_WithoutMonitor_PlansTheSameAddAndReflect_ButNoMonitor()
    {
        ...
        var units = Ext.BuildSyncUnits(
            studios, performers, SlicesFor(videos), alsoMonitor: false, MonitorScope.NewReleases, Options("v3"), Adapter("v3"));

        // Add and monitor are decoupled: the add/reflect legs run regardless of the monitor choice.
        Assert.Equal(new[] { 1 }, CoveIdsOf(units, Ext.SyncOp.ReflectOwned, EntityKind.Studio));
        Assert.Equal(new[] { 10 }, CoveIdsOf(units, Ext.SyncOp.ReflectOwned, EntityKind.Performer));
        Assert.Single(SlicesOf(units));
        Assert.DoesNotContain(units, u => u.Op == Ext.SyncOp.Monitor);
    }

    [Fact]
    public async Task BuildSyncUnits_V2_WithMonitor_RegistersBeforeReflect_StudiosOnly()
    {
        ...
        var units = Ext.BuildSyncUnits(
            studios, performers, SlicesFor(videos), alsoMonitor: true, MonitorScope.NewReleases, Options("v2"), Adapter("v2"));
        ...
        Assert.DoesNotContain(units, u => u.Kind == EntityKind.Performer && u.Op != Ext.SyncOp.AddSlice);
        Assert.DoesNotContain(units, u => u.Op == Ext.SyncOp.AddSlice);
    }

    [Fact]
    public async Task BuildSyncUnits_V2_WithoutMonitor_StillRegistersAndReflects_ButNoMonitor()
    {
        var studios = new[] { Entity(1, null, "t-1"), Entity(2, null, "t-2"), Entity(3, null, null) };
    ...
    [Fact]
    public async Task BuildSyncUnits_EmptyLibrary_PlansNothing()
    {
        var units = Ext.BuildSyncUnits(
    ...
    // Replaces the case that asserted the materialized and streamed builders planned the same units. There is
    // one builder now and it is synchronous, so the divergence that case guarded cannot exist; what stands in
    // its place is the property the single builder must hold — the units it emits for the scene bucket are
    // exactly the slices it was handed, in order.
    [Fact]
    public void BuildSyncUnits_EmitsExactlyTheSlicesItWasGiven_InOrder()
    {
        var videos = Enumerable.Range(100, 1_500).Select(i => Scene(i, $"v-{i}")).ToList();
        var slices = SlicesFor(videos);

        var units = Ext.BuildSyncUnits(
            [], [], slices, alsoMonitor: true, MonitorScope.AllScenes, Options("v3"), Adapter("v3"));

        Assert.Equal(slices, SlicesOf(units));
    }

    // Replaces the case that asserted a unit carried the scene's title. A unit no longer addresses a scene, so
    // what a drawer row must now carry is the slice's scene range — and the id the batch registers has to be
    // distinct from the per-scene form it replaced, or a stale id would collide.
    [Fact]
    public void BuildSyncUnits_LabelsASliceByItsSceneOrdinals()
    {
        var videos = Enumerable.Range(100, 1_500).Select(i => Scene(i, $"v-{i}")).ToList();

        var units = Ext.BuildSyncUnits(
            [], [], SlicesFor(videos), alsoMonitor: false, MonitorScope.NewReleases, Options("v3"), Adapter("v3"));

        Assert.Equal(3, units.Count);
        Assert.Equal([(1, 500), (501, 1_000), (1_001, 1_500)], [.. SlicesOf(units).Select(s => (s.FirstOrdinal, s.LastOrdinal))]);
    }
}
