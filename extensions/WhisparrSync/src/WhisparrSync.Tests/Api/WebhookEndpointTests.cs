...
/// <summary>
/// The connector-sourced /webhook-url read + host-persisting idempotent /register-webhook handler at the
/// endpoint boundary. Injects a <see cref="FakePrincipalAccessor"/> (a Cove.Core
...
public sealed class WebhookEndpointTests
{
    ...
    private static WebhookUrlResponse ReadResponse(IResult result)
        => (WebhookUrlResponse)Assert.IsAssignableFrom<IValueHttpResult>(result).Value!;

    private static bool Registered(IResult result)
        => Assert.IsType<WebhookRegistrationResponse>(
            Assert.IsAssignableFrom<IValueHttpResult>(result).Value).Registered;
    ...
    public async Task WebhookUrl_EmptyList_ReturnsDerivedFromPersistedHostAndRegisteredFalse()
    {
        ...
        // The derived default falls back to the PERSISTED host, not the request host.
    ...
    public async Task RegisterWebhook_ExistingConnection_ReturnsRegisteredTrue_AndPersistsHost()
    {
        ...
        var result = await ext.RegisterWebhookAsync(RequestHost, client, default);
        Assert.True(Registered(result));

        // The resolved origin (the request host here — no override) survives to WhisparrOptions.
