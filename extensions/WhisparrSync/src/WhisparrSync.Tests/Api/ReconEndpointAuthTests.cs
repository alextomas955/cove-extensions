...
/// <summary>
/// Security-critical: the reconciliation endpoints enforce their permission themselves (the
...
/// (Elevation-of-Privilege guard). They also prove confirm/reject validate the submitted ids
/// against the current diff BEFORE writing the map (a forged id writes nothing).
/// </summary>
public sealed class ReconEndpointAuthTests
{
    ...
    // A needs-review diff with one ambiguous-id row: Whisparr movie 10 ↔ Cove video 5. Confirm/reject
    ...
    private static ReconciliationDiff NeedsReviewDiff()
    {
        ...
        var cove = new CoveVideo(
            ...
            StashIds: [], TpdbIds: [], FilePaths: [], Fingerprints: []);
        var row = new MatchResult(movie, cove, MatchedBy.StashId, MatchOutcome.NeedsReview, AutoApplies: false);
    ...
    {
        // EoP: /preview-sync reaches the stored key to call Whisparr, so extensions.read (a strictly
    ...
    {
        // A (coveId, whisparrMovieId) pair absent from the current diff must be refused BEFORE
