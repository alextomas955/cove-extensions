...
public sealed class SceneActionEndpointAuthTests
{
    ...
    private static Task<IResult> Invoke(
        string route, Ext ext, WhisparrClient client, RemoteIdInput[]? remotes = null)
        => route switch
        {
            "scene-add" => ext.SceneAddAsync(new SceneAddRequest(5), client, default),
            "scene-search" => ext.SceneSearchAsync(new SceneSearchRequest(5), client, default),
            "scene-monitor" => ext.SceneMonitorAsync(new SceneMonitorRequest(5, true), client, default),
            "bulk-add-missing" => ext.BulkAddMissingAsync(new BulkAddMissingRequest("studio", 7), client, default),
            "bulk-search-monitored" => ext.BulkSearchMonitoredAsync(
                new BulkSearchMonitoredRequest("studio", remotes ?? MatchingRemotes()), client, default),
    ...
    public async Task BulkSearchMonitored_V2_StashDbOnlyEntity_ReturnsNoIdentity_AndMakesNoOutboundCall()
    {
        ...
        var result = await NewExtension(store).BulkSearchMonitoredAsync(
            new BulkSearchMonitoredRequest("studio", MatchingRemotes()), client, default);
    ...
    public async Task BulkAddMissing_UnknownKind_Returns400()
    {
        ...
        var result = await NewExtension(store).BulkAddMissingAsync(
            new BulkAddMissingRequest("franchise", 7), client, default);
    ...
    public async Task BulkSearchMonitored_UnknownKind_Returns400()
    {
        ...
        var result = await NewExtension(store).BulkSearchMonitoredAsync(
            new BulkSearchMonitoredRequest("franchise", MatchingRemotes()), client, default);
    ...
    public async Task BulkSearchMonitored_NoMatchingStashDbEndpoint_ReturnsNoIdentity_AndMakesNoOutboundCall()
    {
        ...
        var result = await NewExtension(store).BulkSearchMonitoredAsync(
            new BulkSearchMonitoredRequest("studio", NonMatchingRemotes()), client, default);
    ...
    public async Task BulkSearchMonitored_ResponseNeverContainsTheApiKey()
    {
        ...
        var result = await NewExtension(store).BulkSearchMonitoredAsync(
            new BulkSearchMonitoredRequest("studio", MatchingRemotes()), ClientReturning("[]"), default);
