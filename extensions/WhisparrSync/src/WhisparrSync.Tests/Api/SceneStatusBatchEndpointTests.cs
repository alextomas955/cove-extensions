...
/// <see cref="FakeCoveLibraryPort"/> to prove each id classifies with the SAME exclusion-first projector the
/// toolbar summary + scene panel use (a downloaded scene, a not-added scene, and an absent id omitted from the map).
/// </summary>
public sealed class SceneStatusBatchEndpointTests
{
    ...
    [Fact]
    public async Task Core_ClassifiesEachId_DownloadedNotAdded_AndOmitsAbsent()
    {
        // Movie 42 has scene 1's StashDB id and a file → Downloaded; scene 2's id matches no movie → NotAdded;
        // id 3 has no seeded video → absent from the map (the card shows no badge). The core is pure over the
        // pre-fetched movie/exclusion sets (the handler supplies the cached lists) — no fetch, so it cannot grab.
        ...
        Assert.Equal(SceneWhisparrState.Downloaded, states[1]);
        Assert.Equal(SceneWhisparrState.NotAdded, states[2]);
