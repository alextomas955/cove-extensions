...
/** The `/monitor-status` body is either the EntityStatus counts or a `{ code }` outcome. */
type StatusResponse = MonitorStatus | { code: string };
...
function stateFromError(err: unknown): MonitorState {
  if (err instanceof ApiError && isVersionUnsupportedBody(err.body)) {
    return { status: null, noIdentity: false, unsupported: true, error: false, loading: false };
  }
  return { status: null, noIdentity: false, unsupported: false, error: true, loading: false };
}
