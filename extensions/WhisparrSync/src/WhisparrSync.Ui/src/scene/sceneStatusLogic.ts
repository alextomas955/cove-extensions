 ...
 * the offline gate can compile it in isolation exactly like monitorLogic.ts / connectionResult.ts. The
 * `SceneWhisparrState` union lives in contracts.ts and is imported as a type (erased at runtime).
 *
 * `SceneWhisparrState` is the camelCase MIRROR of the C# `SceneWhisparrState` enum's
 * JsonStringEnumConverter output — PINNED to exactly those four strings. It is the management axis
 * (whether Whisparr is monitoring the scene), NOT file presence: a downloaded-and-monitored scene is
 * `monitored`, and `hasFile` travels as a separate secondary fact. Any drift from the wire casing means
 * the panel/toolbar compare the state byte-for-byte against the wrong literal, so these values must stay
 * byte-identical to the server's emitted wire strings — this gate is that drift check.
 */
