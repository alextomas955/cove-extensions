...
import { StatusPill, StatusText } from "@cove-extensions/ui-shared";
...
import { fetchReleaseList, refreshSceneDetail, useSceneDetail } from "./sceneStatusStore";
...
export function WhisparrScenePanel({ entityId }: EntityTabProps) {
  ...
  const { pending, error: actionError, run } = useWhisparrMutation<Exclude<Pending, null>>();
  ...
  if (state.noIdentity) {
    // The missing-id guard: the triggering action is DISABLED with a version-aware, provider-named
    // reason shown both as its aria-label/tooltip AND as a visible inline line — never a dead button. The
    // wording is single-sourced in identityGuardLogic so it cannot drift from the monitor button's copy.
    const reason = missingIdMessage("scene", state.provider);
    const addAffordance = guardedControl({ name: "Add to Whisparr", identityReason: reason });
    return (
      <div className="flex flex-col gap-3 p-4">
        <button
          type="button"
          disabled={addAffordance.disabled}
          title={addAffordance.title}
          aria-label={addAffordance.ariaLabel}
  ...
  // Ahead of the error branch, whose sentence points at the connection: this build answered, and what it
  // answered is that it does not carry the route.
  if (state.capabilityUnavailable) {
    return (
      <div className="p-4">
        <MessageRow text={BUILD_CAPABILITY_COPY} />
      </div>
    );
  }
  ...
  //
  // This deliberately does NOT early-return the way the identity guard does: replacing the panel would take away
  // the read-only status it shows, which is the one thing still worth seeing while a setting is unset.
  ...
  const configRequirement = configIncomplete
    ...
    : null;
  const addAffordance = guardedControl({
    name: "Add to Whisparr",
    enabledTitle: "Add this scene to Whisparr",
    configurationReason: configRequirement,
    busy,
  });
  const monitorAffordance = guardedControl({
    name: control.monitorPressed ? "Stop monitoring this scene" : "Monitor this scene",
    enabledTitle: control.monitorPressed
  ...
  // On success the shared scene detail is refreshed, repainting the badge + controls from a single fetch.
  async function mutate(which: Exclude<Pending, null>, path: string, body: unknown) {
    await run({
      tag: which,
      label: ACTION_LABELS[which],
  ...
  // `grabbingGuid` is the which-row signal the mutation triad does not carry, kept local and cleared once the
  // run settles; the run itself refreshes the shared detail on success.
  async function grab(row: ReleaseRow) {
    ...
    try {
      await run({
        tag: "grab",
        label: ACTION_LABELS.grab,
        versionCapabilityCopy: VERSION_CAPABILITY_COPY,
        action: async () => {
          await request(api("scene-grab-release"), {
            method: "POST",
            body: JSON.stringify(sceneGrabReleaseBody(entityId, guid, row.indexerId)),
          });
          await refreshSceneDetail(entityId);
        },
      });
  ...
  // Removing an exclusion is not destructive, so only the exclude direction styles as one.
  const exclusionPrompt = control.excluded
    ? {
        title: "Remove this exclusion?",
        message: "Whisparr will be allowed to add this scene again on a future import.",
        confirmLabel: "Remove exclusion",
        destructive: false,
      }
    : {
        title: "Exclude this scene from Whisparr?",
        message:
          "It won't be added by future imports until you remove the exclusion. Nothing already in your library is touched.",
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {/* The pre-click reason, on screen rather than only on hover, immediately above the two controls it dims —
          a status region, not the error region above it, because nothing has been attempted yet. */}
      {configSentence !== null && (
        <div role="status">
      ...
      {/* 7. Exclude from Whisparr; reads "Remove exclusion" when already excluded, behind Cove's own
          confirmation modal. */}
      <button
        type="button"
        onClick={() => {
          setConfirmingExclusion(true);
        }}
        disabled={busy}
        title={
          control.excluded
      ...
      <ConfirmDialog
        open={confirmingExclusion}
        title={exclusionPrompt.title}
        message={exclusionPrompt.message}
        confirmLabel={exclusionPrompt.confirmLabel}
        destructive={exclusionPrompt.destructive}
        isPending={pending === "exclude"}
        errorMessage={actionError}
        onConfirm={async () => {
          await mutate(
            "exclude",
            "scene-exclusion",
            sceneExclusionBody(entityId, !control.excluded),
          );
