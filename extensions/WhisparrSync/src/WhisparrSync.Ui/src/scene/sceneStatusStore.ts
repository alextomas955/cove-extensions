...
import type { ReleaseRow } from "../common/lib/sceneActionsLogic";
...
export interface SceneDetailState {
   ...
   * `unsupported` because the remedy differs — a newer build of the same generation — and apart from `error`
   * because no retry can clear it.
   */
...
/** Is a caught failure the connected build declaring none of the needed routes? */
function isCapabilityUnavailable(err: unknown): boolean {
  return err instanceof ApiError && isCapabilityUnavailableBody(err.body);
}
...
function getDetailEntry(coveId: number): ResourceEntry<SceneDetailState> {
  ...
  if (!entry) {
    ...
    pruneIdleEntries(detailEntries);
...
function loadDetail(coveId: number): Promise<void> {
  return runLoad(
    ...
    async () => {
      try {
        const resp = await request<DetailResponse>(api("scene-detail"), {
          method: "POST",
        ...
        return {
          ...
          capabilityUnavailable: isCapabilityUnavailable(err),
          // Both refusals are permanent facts about the connected instance; neither is the generic error a
          // surface renders a retry-shaped sentence for.
          error: !isVersionUnsupported(err) && !isCapabilityUnavailable(err),
          loading: false,
        };
      }
    },
 ...
 */
function clearSceneStatusCaches(): void {
  detailEntries.clear();
  summaryEntry.gen++;
...
registerConnectionScopedCache(clearSceneStatusCaches);
...
function loadSummary(): Promise<void> {
  return runLoad(
    ...
    async () => {
      try {
        const resp = await request<SummaryResponse>(api("scene-status-summary"), { method: "GET" });
        if ("counts" in resp) {
          return {
            counts: resp.counts,
            unsupported: false,
        ...
        return {
          ...
          unsupported: resp.code === "VERSION_UNSUPPORTED",
          capabilityUnavailable: resp.code === "CAPABILITY_UNAVAILABLE",
          error: resp.code !== "VERSION_UNSUPPORTED" && resp.code !== "CAPABILITY_UNAVAILABLE",
          loading: false,
        };
      } catch (err) {
        return {
...
export async function fetchReleaseList(coveId: number): Promise<ReleaseRow[] | null> {
  try {
    const resp = await request<ReleaseListResponse>(api("scene-releases-list"), {
      method: "POST",
      body: JSON.stringify(sceneReleasesBody(coveId)),
    });
