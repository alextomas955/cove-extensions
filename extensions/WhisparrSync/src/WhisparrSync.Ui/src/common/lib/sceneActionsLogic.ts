 ...
 *
 * Kept runtime-import-free (no React, no DOM, no SDK) so the offline gate can compile it in isolation
 * exactly like monitorLogic.ts / sceneStatusLogic.ts. The shared wire unions come from contracts.ts via
 * `import type` (erased at runtime, so the module stays load-clean); the PascalCase request-body shapes
 * mirror the C# request records and stay local.
 *
...
import type { EntityKind, RemoteIdPair, SceneWhisparrState } from "../../contracts";
...
export interface MenuItemsState {
  ...
  /** The bulk items appear/enable only when the entity is monitored (quiet by default). */
  showBulk: boolean;
}
 ...
 * present (labeled + checked when monitored); the bulk items appear only when the entity is monitored
