...
/**
 * The retained-entry ceiling for a per-resource entry map. A library here reaches millions of scenes and a store
 * keys one entry per resource the reader has opened, so an uncapped map grows with a session's browsing rather
 * than with what is mounted.
 */
export const MAX_RESOURCE_ENTRIES = 500;

/**
 * Prunes IDLE entries — no subscriber, no in-flight fetch — oldest first, until `entries` is back within
 * `max`. Returns the number dropped.
 *
 * An entry with a listener or a fetch in flight is never dropped at any size: evicting a subscribed entry would
 * orphan a mounted component's snapshot, and evicting an in-flight one would drop the load its awaiter is
 * holding. The map can therefore sit above `max` when that many resources are genuinely live, which is the
 * honest outcome — the cap bounds a browsing session's residue, not concurrent mounts.
 */
export function pruneIdleEntries<S>(
  entries: Map<unknown, ResourceEntry<S>>,
  max: number = MAX_RESOURCE_ENTRIES,
): number {
  if (entries.size <= max) {
    return 0;
  }
