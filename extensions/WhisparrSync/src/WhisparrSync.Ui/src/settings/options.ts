/**
 * TS mirror of the server-side WhisparrSync options. Models the connection basics and the selected Whisparr
 * version. The Cove-facing `OptionsView` wire is all-camelCase;
 * {@link optionsFromServer} reads the camelCase field names off the response.
 *
...
export interface WhisparrOptions {
  ...
  /** The version string the last successful probe read off the stored host; "" until one succeeds. */
  DetectedVersion: string;
  /**
   * When that version was detected (.NET ticks), or null for never. Stamped only by a successful probe, so it
   * is NOT a reachability time — Whisparr's last-reachable time is a separate line fed by the health record.
   */
   ...
   * that version's URL (and a key-is-set indicator) instead of blanking it. The active version is always present
   * here even before its first per-version save.
   */
...
export function optionsFromServer(raw: unknown): WhisparrOptions {
  ...
  return {
    ...
    DetectedVersion: str(r.detectedVersion, d.DetectedVersion),
    DetectedVersionTicks:
      typeof r.detectedVersionTicks === "number" && Number.isFinite(r.detectedVersionTicks)
        ? r.detectedVersionTicks
        : null,
