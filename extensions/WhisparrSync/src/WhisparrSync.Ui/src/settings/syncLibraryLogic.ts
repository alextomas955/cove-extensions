 ...
 * Kept runtime-import-free (no React, no DOM, no SDK) so the offline gate compiles it standalone, exactly
 * like connectionAvailabilityLogic.ts / entitiesBatchLogic.ts; the `MonitorScope` wire union is imported
 * as a type from contracts.ts (erased at runtime).
 */
