/**
 * Pure, DOM-free logic behind the fixed-option add control. Kept import-free (no React, no DOM, no SDK)
 * so the offline test runner can compile it in isolation exactly like options.ts/primitivesLogic.ts.
 *
 * This is a connection-only extension: the studio/tag entity pickers Renamer shipped here have no
 * consumer, so only the pick-to-add offer logic (`availableOptions`) that the version selector's
 * primitives build on remains.
 */
