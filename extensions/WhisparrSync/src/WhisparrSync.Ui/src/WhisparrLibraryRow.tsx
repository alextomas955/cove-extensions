...
import { Ban, Bookmark, CircleCheck, CircleDashed, Loader } from "lucide-react";
import { GREEN_TINT } from "./hostTintLogic";
...
/** Each state → glyph, glyph color, and pill tint (actionable states tinted, the rest quiet). */
const STATE_PILL: Record<
  SceneWhisparrState,
  { Icon: typeof CircleCheck; icon: string; pill: string; pillStyle?: CSSProperties }
> = {
  downloaded: {
    Icon: CircleCheck,
    icon: "text-green-400",
    pill: "",
    pillStyle: GREEN_TINT,
  },
  monitored: { Icon: Bookmark, icon: "text-accent", pill: "border-accent/40 bg-accent/10" },
  unmonitored: { Icon: CircleDashed, icon: "text-secondary", pill: "border-border bg-card" },
...
/** The states shown as tinted pills in the row's left group, in display order. */
const PILL_ORDER: readonly SceneWhisparrState[] = ["downloaded", "monitored", "excluded"];
