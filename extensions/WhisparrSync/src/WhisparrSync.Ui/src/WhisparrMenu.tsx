 ...
 *   - the quiet "Monitored in Whisparr · X of Y scenes" in-menu status row.
 *
...
export function WhisparrMenu({
  ...
  return createPortal(
    ...
    >
      ...
      {/* Scope sub-options — the "how" of the Monitor toggle; a radio group so exactly one is chosen. */}
      <div className="mt-1 border-t border-border pt-1">
        <div className="px-3 py-1 text-xs font-medium text-secondary">Monitor scope</div>
        ...
        {/* Whisparr-native semantics: raising the scope monitors the back-catalogue, but lowering it (or turning
            monitoring off) leaves already-monitored scenes as they are — say so, so the choice isn't read as a reset. */}
        <p className="px-3 pt-1 text-xs text-secondary">
          Switching to New releases only leaves scenes already monitored as they are.
        </p>
      </div>
      ...
      {/* Quiet in-menu status — "Monitored in Whisparr · X of Y scenes", or nothing when not monitored. */}
      {menu.statusText !== null && (
        <div className="border-t border-border px-3 py-2 text-xs text-secondary">
          {menu.statusText}
        </div>
      )}
    </div>,
