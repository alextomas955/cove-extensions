...
/**
 * The full status-line text: exactly "Monitored in Whisparr · X of Y scenes" — Whisparr's own present-in-
 * library over full-catalog count. The separator is a middot (·) with surrounding spaces. Callers only render
 * this when counts exist; with no counts they render {@link MONITORED_LABEL} bare instead.
 */
export function statusLineText(scenesPresent: number, scenesTotal: number): string {
  return `${MONITORED_LABEL} · ${scenesPresent} of ${scenesTotal} scenes`;
}
