...
import type { SceneWhisparrState } from "./sceneStatusLogic";

const EXTENSION_ID = "com.alextomas955.whisparrsync";
const BATCH_PATH = `/extensions/${EXTENSION_ID}/scene-status-batch`;

// The coalescer keys on the string form of the Cove id; the server keys its `states` map the same way.
const store = createBatchCoalescer<SceneWhisparrState>(async (keys) => {
  const res = await request<{ states?: Record<string, SceneWhisparrState> }>(BATCH_PATH, {
    method: "POST",
...
/**
 * The Whisparr status for one Cove video, resolved via a coalesced batch across all visible cards. Returns null
 * (no badge) until fetched, when `enabled` is false, or when the scene has no resolvable status.
 */
export function useCardStatus(coveId: number, enabled: boolean): SceneWhisparrState | null {
