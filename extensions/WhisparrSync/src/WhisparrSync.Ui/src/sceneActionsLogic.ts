...
export interface MenuItemsState {
  ...
  showBulk: boolean;
  /** The quiet in-menu status line, or null when the entity is not monitored (nothing to say). */
  statusText: string | null;
}

/** The label a monitored entity always shows; the "X of Y scenes" fragment is appended when counts exist. */
...
function hasCounts(status: MonitorStatus): boolean {
 ...
 * (quiet by default); the in-menu status is the "Monitored in Whisparr · X of Y scenes" line when counts
 * exist, the bare label when monitored without counts, and null when not monitored (nothing to say).
 */
export function menuItemsState(status: MonitorStatus | null | undefined): MenuItemsState {
  ...
  if (status?.monitored) {
    statusText = hasCounts(status)
      ? `${MONITORED_LABEL} · ${status.scenesPresent} of ${status.scenesTotal} scenes`
      : MONITORED_LABEL;
  }
  return {
