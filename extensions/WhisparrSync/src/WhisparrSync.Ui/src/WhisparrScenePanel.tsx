...
import { stateBadge, type SceneWhisparrState } from "./sceneStatusLogic";
import {
  releaseSummary,
  sceneAddBody,
  sceneControlState,
...
/** iconKey (from sceneStatusLogic.stateBadge) → its lucide glyph + host accent token (glyph+color, never color-only). */
...
export function WhisparrScenePanel({ entityId }: EntityTabProps) {
  ...
  if (state.noIdentity) {
    return (
      <div className="p-4">
        <MessageRow text="This scene isn't linked to StashDB, so Whisparr has nothing to show for it." />
      </div>
    );
  }
  ...
  const badge = stateBadge(detail.state);
  const visual = STATE_VISUAL[detail.state];
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      {/* Whisparr brand marker — the official Whisparr logo used on every Whisparr surface. */}
      ...
      {/* 1. Status badge header — glyph + label (never color-only). */}
      <div className="flex items-center gap-2">
        <BadgeIcon
          className={`h-5 w-5 ${visual.color}`}
          fill={detail.state === "monitored" ? "currentColor" : "none"}
        />
        <span className="text-base font-semibold text-foreground">{badge.label}</span>
      </div>
