...
export interface ReconRow {
  ...
  matchMethod: string | null; // "StashId" | "Tpdb" | null
  status: string; // "matched" | "needsReview" | "unmatched"
  whisparrState?: string; // "monitored" | "unmonitored" | "notAdded" | "excluded"
  whisparrHasFile?: boolean; // secondary in-library fact; never part of whisparrState
}

/**
 * The four Whisparr management states a reconciliation row can carry, camelCase byte-for-byte as /preview-sync
 * emits them (pinned to the server's emitted wire strings). The axis is Whisparr's monitored flag, not file
 * presence — file presence is the separate optional `whisparrHasFile`. Defined locally rather than imported from
 * sceneStatusLogic so this module stays import-free and file-disjoint from the scene-panel surface — the offline
 * gate compiles it in isolation.
 */
export type WhisparrRowState = "monitored" | "unmonitored" | "notAdded" | "excluded";

/**
 * The legend wording for a row's `whisparrState`. Keyed on the EXACT camelCase wire strings; anything
 * unknown/undefined (a v2 instance that omits the field, a future state) degrades to a neutral "—" rather
 * than a silent blank cell — the row is still legible, it just carries no Whisparr claim.
 */
const WHISPARR_STATE_LABEL: Record<WhisparrRowState, string> = {
  monitored: "Monitored",
...
/** The legend label for a `whisparrState`; unknown/absent → the neutral "—" (never a blank cell). */
export function whisparrStateLabel(state: string | null | undefined): string {
  if (state != null && state in WHISPARR_STATE_LABEL) {
    return WHISPARR_STATE_LABEL[state as WhisparrRowState];
  }
  return "—";
}
 ...
 * - `matched`: a confident (StashDB id / ThePornDB id) auto-link, or a user-confirmed one.
 * - `needs-review`: an id shared by more than one Cove video, awaiting the user's Confirm/Reject — the queue.
...
export type ReconSortColumn = "scene" | "cove" | "method" | "status" | "whisparr";
...
function sortKey(row: ReconRow, column: ReconSortColumn): string {
  switch (column) {
    ...
    case "whisparr":
      // Sort by the human legend wording (so the column orders the way it reads), not the raw wire string.
      return whisparrStateLabel(row.whisparrState).toLowerCase();
  }
}
