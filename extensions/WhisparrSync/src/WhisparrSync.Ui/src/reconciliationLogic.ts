...
export interface ReconRow {
  ...
  whisparrState?: string; // "downloaded" | "monitored" | "unmonitored" | "notAdded" | "excluded"
}

/**
 * The five Whisparr states a reconciliation row can carry, camelCase byte-for-byte as /preview-sync emits
 * them (pinned to the server's emitted wire strings). Defined locally rather than imported from sceneStatusLogic so this module stays
 * import-free and file-disjoint from the scene-panel surface — the offline gate compiles it in isolation.
 */
export type WhisparrRowState = "downloaded" | "monitored" | "unmonitored" | "notAdded" | "excluded";
