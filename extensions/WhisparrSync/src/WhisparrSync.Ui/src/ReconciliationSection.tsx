...
import { Button, SectionGroupHeader, Spinner } from "@cove-ext/ui-shared";
import { FILE_INDICATOR } from "./sceneStatusLogic";
import {
  bucketCounts,
  classifyRow,
  coveItemHref,
...
// Scene | Cove item | Match method | Status | Whisparr | Actions. The six columns share one grid template
// so the sticky header and every virtualized row align. Expressed inline because grid-template-columns with
...
const GRID_TEMPLATE = {
  gridTemplateColumns: "minmax(0,1.5fr) minmax(0,1.5fr) 7rem 9rem 9rem auto",
} as const;
...
/** The label shown on the match-method badge for each resolving id. */
function methodLabel(matchMethod: string | null): string | null {
  if (matchMethod === "StashId") return "StashDB id";
  if (matchMethod === "Tpdb") return "ThePornDB id";
  return null;
}
...
// The per-state glyph + host color token for the Whisparr status badge — matched to the scene-panel /
// library-toggle visuals for a consistent legend across surfaces, but declared locally here so this
// file stays disjoint from sceneStatusLogic. Each state is a distinct glyph so status is never color-only.
const WHISPARR_STATE_VISUAL: Record<WhisparrRowState, { Icon: typeof Bookmark; color: string }> = {
...
/** The SECONDARY in-library dot painted beside the primary state badge when the row carries a file. */
function FileDot() {
  return (
    <Download className="h-3.5 w-3.5 text-green-400" aria-label={FILE_INDICATOR.label} role="img" />
  );
}

/**
 * The Whisparr status badge for a reconciliation row — the four-state management axis. A glyph + the legend
 * label so status is NEVER color-only (the StatusPill rule); a secondary file dot is painted alongside when the
 * row has a file (never part of the state). Excluded gets a distinct red-tinted pill so the exclusion state is
 * legible at a glance; the other states sit on the neutral shell. An unknown/absent state (a v2 row that omits
 * it) degrades to a muted "—" rather than a blank cell.
 */
function WhisparrStateBadge({ state, hasFile }: { state: string | undefined; hasFile?: boolean }) {
  const base = "inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs font-medium";
  const label = whisparrStateLabel(state);
  const dot = hasFile === true ? <FileDot /> : null;
  if (state == null || !(state in WHISPARR_STATE_VISUAL)) {
    return (
      <span className="inline-flex items-center gap-1.5">
        <span className={`${base} border-border bg-card text-muted`}>
  ...
  const { Icon, color } = WHISPARR_STATE_VISUAL[state as WhisparrRowState];
  const pill =
    state === "excluded"
...
/** The accent match-method badge (StashDB id / ThePornDB id); nothing for an unmatched row. */
            ...
            {visible.length === 0 ? (
              <p className="py-8 text-center text-sm text-secondary">
                {search
                  ...
                  : filter === "needs-review"
                    ? "No matches need review. An id shared by more than one Cove video lands here for you to confirm or reject."
                    : filter === "all"
                      ? "No scenes to show."
              ...
              <div className="overflow-hidden rounded border border-border text-sm">
                {/* Sticky grid header — shares GRID_TEMPLATE with every body row. Five sortable columns + actions. */}
                <div
                    ...
                    {rowVirtualizer.getVirtualItems().map((vRow) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="px-3 py-2">
                            <WhisparrStateBadge
                              state={row.whisparrState}
                              hasFile={row.whisparrHasFile}
                            />
                          </span>
