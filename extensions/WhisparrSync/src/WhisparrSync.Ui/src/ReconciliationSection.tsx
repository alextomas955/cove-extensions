...
import { Button, SectionGroupHeader, Spinner } from "@cove-extensions/ui-shared";
import { AMBER_TINT, RED_TINT } from "./hostTintLogic";
import {
  bucketCounts,
  classifyRow,
  coveItemHref,
...
function StatusPill({ segment }: { segment: ReconSegment }) {
  ...
  if (segment === "needs-review") {
    return (
      <span className={`${base} text-amber-400`} style={AMBER_TINT}>
  ...
  return (
    <span className={`${base} border-red-700/50 text-red-400`} style={RED_TINT}>
...
const WHISPARR_STATE_VISUAL: Record<WhisparrRowState, { Icon: typeof CircleCheck; color: string }> =
  {
...
/**
 * The Whisparr status badge for a reconciliation row — the per-scene 4-state fallback list.
 * A glyph + the legend label so status is NEVER color-only (the StatusPill rule). Excluded gets a
 * distinct red-tinted pill so the exclusion state is legible at a glance; the other states sit on the neutral
 * shell. An unknown/absent state (a v2 row that omits it) degrades to a muted "—" rather than a blank cell.
 */
function WhisparrStateBadge({ state }: { state: string | undefined }) {
  const base = "inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs font-medium";
  ...
  return (
    <span className={`${base} border-border bg-card ${color}`}>
      <Icon className="h-3 w-3" aria-hidden />
      {label}
    </span>
  );
                    ...
                    {rowVirtualizer.getVirtualItems().map((vRow) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="px-3 py-2">
                            <WhisparrStateBadge state={row.whisparrState} />
                          </span>
