...
export function WhisparrStatusLine(props: SlotProps) {
  ...
  if (state.error || state.capabilityUnavailable) {
    return (
      <div role="status" aria-live="polite" className="flex items-center gap-2 text-sm">
        <AlertTriangle className="h-4 w-4 text-amber-400" />
        <StatusText kind="warning">
          {/* A build that answered "I don't carry that route" is not an unreachable instance, and the
              connection sentence would send the user to check a setting that is already correct. */}
          {state.capabilityUnavailable ? BUILD_CAPABILITY_COPY : WHISPARR_UNAVAILABLE_COPY}
        </StatusText>
      </div>
    );
  }
