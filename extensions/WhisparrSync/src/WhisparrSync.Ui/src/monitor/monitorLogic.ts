 ...
 * so the offline gate can compile it in isolation exactly like connectionResult.ts. The wire unions
 * (EntityKind, RemoteIdPair, MonitorScope) are declared once in contracts.ts and imported as types;
 * `import type` erases at runtime, so the module stays load-clean for the gate.
 */

import type { EntityKind, MonitorScope, RemoteIdPair } from "../contracts";
import { VERSION_CAPABILITY_COPY, WHISPARR_UNAVAILABLE_COPY } from "../common/lib/whisparrCopy";

export { VERSION_CAPABILITY_COPY, WHISPARR_UNAVAILABLE_COPY };
 ...
 * that "All scenes" also queues the existing back-catalogue, and that switching
