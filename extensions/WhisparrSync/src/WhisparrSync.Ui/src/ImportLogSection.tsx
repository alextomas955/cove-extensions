...
import { AMBER_TINT } from "./hostTintLogic";
import {
  bucketCounts,
  classifyRow,
  coveItemHref,
...
export function ImportLogSection() {
  ...
  // Default to the "When" column descending so the newest imports lead on open. The server returns the journal
  // in append (oldest-first) order and the body is virtualized, so without this the latest events sort to the
  // bottom, off-screen — the opposite of what the user wants to see first.
  const [sortColumn, setSortColumn] = useState<ImportSortColumn | null>("when");
