...
const { createBatchCoalescer, MAX_CACHED_ENTRIES } = mod;
...
test("the cache is bounded: past the ceiling the least recently used key is dropped", async () => {
  const fetched = [];
  const { store, settle } = harness(async (keys) => {
    fetched.push(...keys);
    return new Map(keys.map((k) => [k, k]));
  ...
  for (let i = 0; i < MAX_CACHED_ENTRIES + 1; i++) {
    store.request(`k${i}`);
  }
  await settle();

  // Every key was fetched, but the first is no longer retained — scrolling a million-scene library cannot grow
  // browser state without limit.
  assert.equal(fetched.length, MAX_CACHED_ENTRIES + 1);
  assert.equal(store.get("k0"), null);
  assert.equal(store.get(`k${MAX_CACHED_ENTRIES}`), `k${MAX_CACHED_ENTRIES}`);

  fetched.length = 0;
  store.request("k0");
  await settle();
  assert.deepEqual(fetched, ["k0"]); // evicted, so it re-fetches
});

test("requesting a cached key marks it recently used so it survives an eviction", async () => {
  const { store, settle } = harness(async (keys) => new Map(keys.map((k) => [k, k])));

  store.request("keep");
  await settle();

  // "keep" is the OLDEST entry, so without the recency touch it would be the first evicted.
  for (let i = 0; i < MAX_CACHED_ENTRIES - 1; i++) {
    store.request(`f${i}`);
  }
  await settle();
  store.request("keep"); // a card scrolled back into view
  store.request("overflow");
  await settle();

  assert.equal(store.get("keep"), "keep");
  assert.equal(store.get("f0"), null); // the next-oldest went instead
});
