#!/usr/bin/env node
/**
 * Offline logic gate for the monitor UI logic, in the spirit of check-reconciliation-logic.mjs: no new
 * dependency, no test framework beyond Node's built-in runner. monitorLogic.ts is a pure module with zero
...
const logicTs = path.join(uiRoot, "src", "monitorLogic.ts");
const testFile = path.join(here, "monitor-logic.test.mjs");

const outDir = mkdtempSync(path.join(tmpdir(), "whisparr-monitor-"));

function run() {
  // monitorLogic.ts is a pure module with zero imports, so compile it in isolation. `types: []` keeps tsc
  ...
  if (compile.status !== 0) {
    console.error("check-monitor-logic: tsc compile FAILED");
    return compile.status ?? 1;
  }

  const compiledModule = pathToFileURL(path.join(outDir, "monitorLogic.js")).href;
  const test = spawnSync(process.execPath, ["--test", testFile], {
    stdio: "inherit",
    env: { ...process.env, MONITOR_LOGIC_MODULE: compiledModule },
  });
