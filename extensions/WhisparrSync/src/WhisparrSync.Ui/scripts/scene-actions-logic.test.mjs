...
test("sceneControlState truth table: add/monitor/search/interactive/upgrades/exclude derivations", () => {
  ...
  // downloaded (added, monitored) → search + interactive enabled, monitor + upgrades pressed.
  assert.deepEqual(sceneControlState({ state: "downloaded", added: true, monitored: true }), {
    showAdd: false,
    monitorPressed: true,
    searchEnabled: true,
    interactiveAvailable: true,
...
test("menuItemsState: monitored with counts → checked + bulk shown + full status line", () => {
  const state = menuItemsState({ added: true, monitored: true, scenesPresent: 12, scenesTotal: 40 });
  assert.deepEqual(state, {
    monitorLabel: "Monitor in Whisparr",
    monitorChecked: true,
    showBulk: true,
    statusText: "Monitored in Whisparr · 12 of 40 scenes",
  });
  assert.equal(MONITOR_MENU_LABEL, "Monitor in Whisparr");
});

test("menuItemsState: monitored without counts → bare label; honors hasCounts:false", () => {
  assert.equal(
    menuItemsState({ added: true, monitored: true, scenesPresent: 0, scenesTotal: 0 }).statusText,
    MONITORED_LABEL,
  );
  assert.equal(
    menuItemsState({ added: true, monitored: true, scenesPresent: 0, scenesTotal: 0, hasCounts: false })
      .statusText,
    MONITORED_LABEL,
  );
...
test("menuItemsState: not monitored (or null) → unchecked, bulk hidden, statusText null", () => {
