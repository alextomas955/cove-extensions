...
test("isCapabilityUnavailableBody: the build-capability code, distinct from the version code", () => {
  const { isCapabilityUnavailableBody, CAPABILITY_UNAVAILABLE } = mod;
  assert.equal(isCapabilityUnavailableBody(`{"code":"${CAPABILITY_UNAVAILABLE}"}`), true);
  assert.equal(isCapabilityUnavailableBody(`{"code":"${VERSION_UNSUPPORTED}"}`), false);
  assert.equal(isVersionUnsupportedBody(`{"code":"${CAPABILITY_UNAVAILABLE}"}`), false);
  assert.equal(isCapabilityUnavailableBody('{"code":"capability_unavailable"}'), false);
  assert.equal(isCapabilityUnavailableBody("502 Bad Gateway"), false);
  assert.equal(isCapabilityUnavailableBody(""), false);
});

test("the two codes are pinned wire literals and are not each other", () => {
  const { CAPABILITY_UNAVAILABLE } = mod;
  assert.equal(CAPABILITY_UNAVAILABLE, "CAPABILITY_UNAVAILABLE");
  assert.notEqual(CAPABILITY_UNAVAILABLE, VERSION_UNSUPPORTED);
});
