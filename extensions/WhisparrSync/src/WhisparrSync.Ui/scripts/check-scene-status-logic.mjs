#!/usr/bin/env node
/**
 * Offline logic gate for the scene-status UI logic, in the spirit of check-monitor-logic.mjs: no new
 * dependency, no test framework beyond Node's built-in runner. sceneStatusLogic.ts is a pure module with zero
...
const logicTs = path.join(uiRoot, "src", "sceneStatusLogic.ts");
const testFile = path.join(here, "scene-status-logic.test.mjs");

const outDir = mkdtempSync(path.join(tmpdir(), "whisparr-scene-status-"));

function run() {
  // sceneStatusLogic.ts is a pure module with zero imports, so compile it in isolation. `types: []` keeps tsc
  ...
  if (compile.status !== 0) {
    console.error("check-scene-status-logic: tsc compile FAILED");
    return compile.status ?? 1;
  }

  const compiledModule = pathToFileURL(path.join(outDir, "sceneStatusLogic.js")).href;
  const test = spawnSync(process.execPath, ["--test", testFile], {
    stdio: "inherit",
    env: { ...process.env, SCENE_STATUS_LOGIC_MODULE: compiledModule },
  });
