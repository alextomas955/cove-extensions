...
test("whisparrStateLabel maps each pinned camelCase wire state to its DESIGN legend wording", () => {
  // Checker W1: these are the EXACT camelCase strings /preview-sync emits (pinned in 07-02). Each must map
  // to a real label — NOT the unknown "—" fallback — or the column silently blanks every populated row.
  assert.equal(whisparrStateLabel("downloaded"), "Downloaded");
