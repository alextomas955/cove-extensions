...
test("classifyActionFailure: 400 discriminates noIdentity / versionUnsupported / configIncomplete / unknown", () => {
  ...
  assert.equal(
    classifyActionFailure(400, '{"code":"CONFIG_INCOMPLETE","options":["qualityProfileId"]}'),
    "configIncomplete",
  );
...
test("configIncomplete returns the caller's setting-naming sentence, and never the generic fallback", () => {
  const named = "Pick a quality profile in Whisparr Sync settings (Add defaults) before adding.";
  assert.equal(actionFailureMessage("configIncomplete", "cap-copy", null, named), named);

  // With no caller copy the sentence must still point at a setting rather than collapsing to "unknown".
  const bare = actionFailureMessage("configIncomplete", "cap-copy");
  assert.notEqual(bare, actionFailureMessage("unknown", "cap-copy"));
  assert.equal(bare.toLowerCase().includes("setting"), true);

  assert.equal(
    actionFailureCopy(
      "update monitoring",
      400,
      '{"code":"CONFIG_INCOMPLETE","options":["qualityProfileId"]}',
      "cap-copy",
      named,
    ),
    `Couldn't update monitoring — ${named}`,
  );
});

test("configIncomplete: the refusal body's option keys never reach the rendered sentence", () => {
  const copy = actionFailureCopy(
    "update monitoring",
    400,
    '{"code":"CONFIG_INCOMPLETE","options":["baseUrl","apiKey"]}',
    "cap-copy",
  );
  for (const wireName of ["baseUrl", "apiKey", "options", "CONFIG_INCOMPLETE"]) {
    assert.equal(copy.includes(wireName), false, `must not leak the wire name "${wireName}"`);
  }
});
    ...
    const [status, body] =
      kind === "noIdentity"
        ...
        : kind === "versionUnsupported"
          ...
          : kind === "configIncomplete"
            ? [400, '{"code":"CONFIG_INCOMPLETE","options":["qualityProfileId"]}']
            : kind === "unknown"
...
test("a build missing a route classifies to its own kind, distinct from unknown and versionUnsupported", () => {
  const kind = classifyActionFailure(400, '{"code":"CAPABILITY_UNAVAILABLE"}');
  // Pairwise, because a new kind that merely aliases an existing one passes a non-null check.
  assert.equal(kind, "capabilityUnavailable");
  assert.notEqual(kind, "unknown");
  assert.notEqual(kind, "versionUnsupported");
  assert.notEqual(classifyActionFailure(400, '{"code":"VERSION_UNSUPPORTED"}'), kind);
  assert.notEqual(classifyActionFailure(400, '{"code":"ANYTHING_ELSE"}'), kind);
});

test("the capability refusal reads as permanent and never invites a retry", () => {
  const message = actionFailureMessage("capabilityUnavailable", "cap-copy", null, null, "build-copy");
  assert.equal(message, "build-copy");
  assert.notEqual(message, actionFailureMessage("unknown", "cap-copy"));
  assert.notEqual(message, actionFailureMessage("versionUnsupported", "cap-copy"));

  // With no caller copy it still must not fall through to the retry sentence.
  const bare = actionFailureMessage("capabilityUnavailable", "cap-copy");
  assert.doesNotMatch(bare, /try again/i);
  assert.doesNotMatch(bare, /moment/i);
  assert.notEqual(bare, actionFailureMessage("unknown", "cap-copy"));

  const line = actionFailureCopy(
    "load the missing list",
    400,
  ...
  assert.equal(line, "Couldn't load the missing list — build-copy");
});
