...
> **Status:** 0.1.0. You enter a URL + key, test the connection, set your add defaults from an
> auto-populated dropdown, and generate (and optionally auto-register) a webhook URL. It auto-imports
> what Whisparr grabs (a webhook plus a polling backstop that re-reads Whisparr's import history since
> a stored checkpoint and feeds anything it hasn't seen through the same import path), and lets you push
> to Whisparr — monitor a studio, performer, or scene, and add or search in bulk — with an opt-in
> per-scene Whisparr status view and a per-entity Missing tab.

After a successful test the page shows a ready-to-use **webhook URL** with an embedded secret that you can
copy into Whisparr or let the extension add for you. When Whisparr
...
monitored / unmonitored / not added / excluded, plus a second marker when Whisparr holds the file;
studios & performers: a "Monitored · present/catalog" count), derived from the reconciliation movie set
plus an exclusion read — no StashDB calls. One toolbar pill reveals the badges and a library-level count
row; status also appears in the scene detail Whisparr tab. See the
[Whisparr status reference](https://alextomas955.github.io/cove-extensions/extensions/whisparr-sync/status).

A per-entity **Missing** tab on studio, performer, and tag pages surfaces what they offer that Cove
doesn't own yet — the entity's catalogue minus what you own minus what you've excluded in Whisparr — as
a card grid mirroring Cove's native videos card (cover, performer avatars, tags, description, and an
always-on Whisparr status). A live count badge tracks the total; the grid shows one page at a time with
paging controls below it; sort and filter by performer/studio/tag/year are asked of the metadata source
rather than applied to the loaded page; and you can mark scenes wanted, unmonitor, or search per card or
over a multi-selection (bulk actions run as a background job and report no per-scene outcome on the
page). The studio, performer and tag tabs all work on v3 and v2; the per-scene controls are v3 only. The
catalogue is read directly from the metadata source Cove already has configured — StashDB on v3,
ThePornDB on v2 — so
there are no extra credentials to enter. A separate, read-only
**Wanted, queue & history** page tracks what Whisparr is looking for, downloading, and has acquired. See
[Find the scenes you're missing](https://alextomas955.github.io/cove-extensions/extensions/whisparr-sync/find-missing-scenes),
the [Missing tab reference](https://alextomas955.github.io/cove-extensions/extensions/whisparr-sync/discovery),
and [Wanted, queue & history](https://alextomas955.github.io/cove-extensions/extensions/whisparr-sync/activity).

## Whisparr v2

Both **v3 (Eros)** and **v2** are supported. On v2 the extension connects, imports what Whisparr
acquires (webhook + polling backstop), and derives its read-only status from a **single id-only match**
— the same rule v3 uses, keyed on a different id.

That id is the one difference: Whisparr v2 is ThePornDB-native and carries **no StashDB id** on any
scene, so the StashDB-id match v3 leads with cannot apply. This is a permanent property of v2's data
model rather than a missing feature — the match keys on the **ThePornDB** id instead, which is equally
durable, so there is no fuzzy title-or-year fallback and no bridge left to build. See
...
What works on v2: connect, import (webhook + polling backstop), the read-only status derivation over the
reconciliation movie set, the per-entity Missing tab, the Wanted / queue / history page, and the
studio-monitor / status / search outward path above — see
[Architecture](./docs/ARCHITECTURE.md#whisparr-v2-adapter).
...
- **[Connect guide](https://alextomas955.github.io/cove-extensions/extensions/whisparr-sync/guide)** — connect, set your add defaults, add the webhook
...
- **[Find the scenes you're missing](https://alextomas955.github.io/cove-extensions/extensions/whisparr-sync/find-missing-scenes)** — open an entity's Missing tab, narrow the list, and mark scenes wanted or grab them
- **[Missing tab reference](https://alextomas955.github.io/cove-extensions/extensions/whisparr-sync/discovery)** — every part of the per-entity Missing tab: the scene cards, filters, bulk actions, states, and sourcing
- **[Wanted, queue & history](https://alextomas955.github.io/cove-extensions/extensions/whisparr-sync/activity)** — the read-only page tracking what Whisparr is looking for, downloading, and has acquired
- **[Reconciliation](https://alextomas955.github.io/cove-extensions/extensions/whisparr-sync/reconciliation)** — the 15-minute import backstop behind the webhook: what it reads, what it writes, and how a Cove item is matched to a Whisparr scene
...
- `src/WhisparrSync/` — the C# extension (`FullExtensionBase`): identity, the full-page settings tab
  manifest, the settings endpoints (`/test-connection`, `/status`, `/options`,
  `/webhook-url`, `/register-webhook`), the capability slices (`Ingest/`, `Matching/`, `Monitor/`,
  `Push/`, `SceneStatus/`, `Activity/`, `Discovery/`), the version-adapter seam (`Adapters/`), the
  options store (`Options/`), the root-read and overlap seam (`Safety/`), and the transport-only
  `Client/WhisparrClient`. `docs/ARCHITECTURE.md` carries the full endpoint table.
