...
public sealed class PreviewEndpointTests
{
    ...
    {
        // A typed Ok<T> serializes through HttpContext.RequestServices, so a bare DefaultHttpContext
        // throws before writing a byte. The enum spelling under test comes from the converter on the
        // enum TYPE and so is unaffected by which options instance this provider hands over.
