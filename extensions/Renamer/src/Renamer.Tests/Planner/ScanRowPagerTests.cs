...
/// handed. What is at stake in that last case is the composition — that the page computes the flag at
/// all, and against the same budget the planner just planned against.
/// </summary>
[Trait("Tier", "L0")]
public sealed class ScanRowPagerTests
{
    ...
    private static RenamerFile OverflowVideoFile(int fileId) =>
    ...
    private static RenamerEntity OverflowEntity(int entityId, string title, int studioId) =>
        new(EntityId: entityId, Kind: RenamerFileKind.Video, Title: title, Code: null,
            ...
            Files: [OverflowVideoFile(entityId)], StudioId: studioId);
