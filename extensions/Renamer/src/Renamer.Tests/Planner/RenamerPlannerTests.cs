...
public sealed class RenamerPlannerTests
{
    ...
    public async Task PlanLoadedEntityAsync_MatchesLoadingPath_ItemForItem()
    {
        ...
        var loaded = await planner.PlanLoadedEntityAsync(entity, opts, EmptyLookups, default);
    ...
    public async Task PlanLoadedEntityAsync_PerformsZeroLoads()
    {
        ...
        var plan = await planner.PlanLoadedEntityAsync(entity, new RenamerOptions { FilenameTemplate = "$title" }, EmptyLookups, default);
