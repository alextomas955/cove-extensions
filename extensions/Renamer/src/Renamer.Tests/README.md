...
suite is the L3 gate; a behavior needing L3 gets an e2e spec, never a fourth trait value. Since
`.runsettings` sets `TreatNoTestsAsError`, `--filter "Tier=L3"` fails the run rather than reporting a
green nothing, which is the correct answer to asking this project for a tier it does not have.
...
expected to _fail_ anywhere. Each is a `[SkippableFact]` whose `Skip.IfNot` names the facility it
needs, so off that filesystem it skips visibly with a reason and the run stays green: the Linux
