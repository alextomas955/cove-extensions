 ...
 *
 * A response is shown only while the generation it was issued under is still the current one.
 * Anything else describes options the user has already moved past, and rendering it puts a name in the
 * pane that the settings no longer produce.
 *
 ...
 * aborts what it supersedes, so reading that as a failure would report one the hook caused itself
 * while a healthy request is still on its way.
 */
