...
/// <c>RenamerOptions</c> — see <c>RenamerOptions.JsonOptions</c> for why typed binding cannot take
/// this body. Deserializing the string with those same options (the contract
/// <see cref="global::Renamer.Options.OptionsStore"/> persists) keeps the scan's parsing
/// byte-identical to the saved-options load path, so a dry run on unsaved edits and a scan of saved
/// settings interpret the blob the same way. A null or blank blob means "use the saved options" — the
/// original no-body behavior, preserved for back-compat.
/// </remarks>
