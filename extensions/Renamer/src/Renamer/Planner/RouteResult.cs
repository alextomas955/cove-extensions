...
public enum RouteCategory
{
    ...
    /// <summary>A rule matched on a stable tag id, in entity tag-list order; the label still names the tag.</summary>
    Tag,
    ...
    /// <summary>
    /// No rule matched, so the item's destination is the DEFAULT
    /// (<c>RenamerOptions.FolderRoot</c> + <c>RenamerOptions.FolderTemplate</c>, the panel's
    /// <i>Where files go</i>). <see cref="RouteResult.Destination"/> is null for this category — not
    /// because the item stays put, but because no rule supplied a destination and the default is the
    /// planner's to read.
    /// </summary>
    Unmatched,
}
...
/// The matched rule's own destination — its whole answer, root and relative template together, not a
/// root some other setting is then rendered underneath. <c>null</c> for
/// <see cref="RouteCategory.Unmatched"/>/<see cref="RouteCategory.Excluded"/>: an unmatched item takes
/// the default destination instead, and an excluded one is never rendered at all.
/// </param>
/// <remarks>
/// One lookup, one destination. The planner asks "where does this file go?" exactly once and renders a
/// single answer, which is what makes a destination stable under the move it names: two user-authored
...
/// Stable tag-id exclude set, keyed on the id exactly like <see cref="TagIdToDest"/>. An entity
/// carrying any tag in this set is excluded FIRST (before every routing category). Empty (the
/// default) = no tag excludes.
/// </param>
