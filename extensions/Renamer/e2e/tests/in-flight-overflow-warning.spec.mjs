...
async function raiseConfirmGate(page, videosPage, cardName) {
  ...
  try {
    ...
    // The deadline is a cleared timer rather than waitForTimeout: the losing arm of a race outlives it,
    // so a fixed wait would still be pending when the gate fires and would then throw into a promise
    ...
    try {
      await Promise.race([
        ...
        new Promise((_resolve, reject) => {
          deadline = setTimeout(
            () => reject(new Error(`the confirm gate never fired for "${cardName}" within 15s`)),
            15_000,
          );
        }),
      ]);
