...
// cheapest file that carries it, selected with `--grep @smoke`. It is a SELECTION, never a tier: a
// tagged test has no other privilege and still runs in the full suite. Three of the six are here, so
// membership is per test rather than per file. The tag is passed as the test's OPTION rather than
// written into its title, because a `--grep` on an unrelated word also matches a title-embedded tag,
// which would widen or narrow the set with nothing saying so. The first test below is a member by
// obligation, not by judgement: it is the standing observation point for "the produced package
// installs into a clean released host and reaches enabled state", so no later axis change can drop
// that observation without deleting a tag here.
