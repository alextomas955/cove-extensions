...
const REAL_LINE =
  "Required references did not load for Renamer.Tests or referenced project. Run `dotnet restore` prior to formatting.";

test("names the project the tool reported", () => {
  assert.deepEqual(projectsWithUnloadedReferences(REAL_LINE), ["Renamer.Tests"]);
});
...
test("finds the sentence among the tool's other output", () => {
  ...
  assert.deepEqual(projectsWithUnloadedReferences(output), ["Renamer.Tests"]);
});
...
test("does not guess a project name from a line that is not the tool's sentence", () => {
  assert.deepEqual(
    projectsWithUnloadedReferences(
      "Some required references did not load for Renamer.Tests or referenced project.",
    ),
    [],
  );
  assert.deepEqual(
    projectsWithUnloadedReferences("Required references did not load for Renamer.Tests."),
    [],
  );
});
