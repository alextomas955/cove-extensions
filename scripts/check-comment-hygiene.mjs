...
const TELLS = [
  ...
  { re: /\([A-Z]{2,12}-\d{1,3}\)|\b[A-Z]{2,12}-\d{1,3}(?:'s\b|\s+(?:says|requires|covers))/, why: "requirement id" },
];
...
if (diff.trim() === "") {
  console.error(`comment-hygiene: NO INPUT — ${mode} diff over *.cs/*.ts/*.tsx is empty, so nothing was inspected.`);
