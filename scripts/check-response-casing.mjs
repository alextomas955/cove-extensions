...
for (const policyDir of POLICY) {
  for (const file of walk(join(root, policyDir))) {
    ...
    while ((m = callRe.exec(src))) {
      ...
      // The DECLARATION of one of these helpers is not a call: its angle brackets hold a type
      // PARAMETER, which names no response shape and can never resolve to a property list. Reading it
      // as a call site would report the transport itself as a permanent blind spot.
      if (/\b(?:function|const|let|var|declare)\s+$/.test(src.slice(Math.max(0, m.index - 40), m.index))) {
        continue;
      }
